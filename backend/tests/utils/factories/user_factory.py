...
class UserFactory(BaseFactory):
    ...
    async def create_admin(self, **kwargs) -> User:
        ...
        data = {"role": UserRole.SUPER_ADMIN, "department": "管理部"}
        data.update(kwargs)
        return await self.create(**data)

    async def create_manager(self, **kwargs) -> User:
        """Create a manager user"""
        data = {"role": UserRole.MANAGER, "department": "管理部"}
        data.update(kwargs)
        return await self.create(**data)

    async def create_office_staff(self, **kwargs) -> User:
        """Create an office staff user"""
        data = {"role": UserRole.OFFICE_STAFF, "department": "營運部"}
        data.update(kwargs)
        return await self.create(**data)
    ...
    async def create_customer_user(self, **kwargs) -> User:
        ...
        data = {"role": UserRole.CUSTOMER, "department": None, "employee_id": None}
        data.update(kwargs)
        return await self.create(**data)
