...
class OrderFactory(BaseFactory):
    ...
    async def get_default_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "payment_method": self.fake.random_element(
                ["cash", "credit_card", "bank_transfer", "monthly_billing"]
            ),
            "created_by": 1,
            "updated_by": 1,
            "route_id": None,
    ...
    async def create_in_delivery(
        self, customer_id: int, driver_id: int, **kwargs
    ) -> Order:
    ...
    async def create_urgent(self, customer_id: int, **kwargs) -> Order:
        ...
        data = {
            ...
            "is_urgent": True,
            "scheduled_date": datetime.now()
            + timedelta(hours=self.fake.random_int(1, 6)),
    ...
    async def create_with_specific_products(
        ...
        **kwargs,
        ...
        if "discount_percentage" in kwargs:
            discount_amount = round(
                total_amount * kwargs["discount_percentage"] / 100, 2
            )
