...
class CustomerFactory(BaseFactory):
    ...
    async def get_default_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "area": self.fake.random_element(['信義區', '大安區', '中山區', '松山區', '內湖區', '南港區', '文山區', '萬華區']),
            ...
            "current_credit": 0.0,
            "payment_terms": self.fake.random_element([0, 30, 45, 60]) if is_corporate else 0,
            "bank_name": self.fake.random_element(['台灣銀行', '合作金庫', '第一銀行', '華南銀行', '彰化銀行']) if is_corporate else "",
            "bank_account": f"{self.random_string(14, '0123456789')}" if is_corporate else "",
            "notes": "",
            "created_by": 1,
            "updated_by": 1
        }
    ...
    async def create_restaurant(self, **kwargs) -> Customer:
        ...
        restaurant_name = f"{self.fake.word()}{self.fake.random_element(restaurant_types)}"
        data = {
    ...
    async def create_terminated(self, **kwargs) -> Customer:
        ...
        data = {
            "is_terminated": True,
            "is_active": False,
            "termination_date": datetime.now() - timedelta(days=self.fake.random_int(1, 365)),
            "termination_reason": self.fake.random_element(['搬遷', '結業', '改用其他供應商', '長期未訂購'])
        }
        data.update(kwargs)
        return await self.create(**data)
    ...
    async def create_subscription(self, **kwargs) -> Customer:
        ...
        data = {
            ...
            "subscription_frequency": self.fake.random_element(['weekly', 'biweekly', 'monthly']),
            "subscription_day": self.fake.random_int(1, 7) if kwargs.get('subscription_frequency') == 'weekly' else self.fake.random_int(1, 28),
            "notes": "定期配送客戶"
        }
        data.update(kwargs)
        return await self.create(**data)