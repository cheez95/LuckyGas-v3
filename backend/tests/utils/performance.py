...
class PerformanceMonitor:
            ...
            "operations": [
                {
                    ...
                    "percentiles": {
                        "min": (
                            round(m.min_duration_ms, 2) if m.min_duration_ms else None
                        ),
                        "avg": (
                            round(m.avg_duration_ms, 2) if m.avg_duration_ms else None
                        ),
                        "p95": (
                            round(m.p95_duration_ms, 2) if m.p95_duration_ms else None
                        ),
                        "p99": (
                            round(m.p99_duration_ms, 2) if m.p99_duration_ms else None
                        ),
                        "max": (
                            round(m.max_duration_ms, 2) if m.max_duration_ms else None
                        ),
                    },
                }
    ...
    def assert_performance(
        ...
        if max_memory_mb:
            assert (
                abs(metric.memory_delta_mb) <= max_memory_mb
            ), f"{operation} used {metric.memory_delta_mb:.2f}MB, expected <= {max_memory_mb}MB"

        if max_cpu_percent:
            assert (
                metric.cpu_percent <= max_cpu_percent
...
class LoadTester:
    ...
    async def run_load_test(
        self, func: Callable, duration_seconds: int = 60, rps: int = 10, *args, **kwargs
    ) -> Dict[str, Any]:
        ...
        if durations:
            return {
                ...
                "error_rate": (
                    failed / (successful + failed) if (successful + failed) > 0 else 0
                ),
                "latency": {
                    ...
                    "p99_ms": (
                        round(sorted(durations)[int(len(durations) * 0.99)], 2)
