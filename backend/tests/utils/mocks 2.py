...
class MockGoogleRoutesService(MockService):
    ...
    async def optimize_route(self, stops: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        return self._get_response("optimize_route", {
            "total_distance_km": len(stops) * 2.5,
            "total_duration_minutes": len(stops) * 15,
            "optimized_stops": stops,
            "route_polyline": "mock_polyline_" + ''.join(random.choices(string.ascii_lowercase, k=20)),
            "optimization_score": 0.85
        })
    
    async def get_distance_matrix(self, origins: List[str], destinations: List[str]) -> Dict[str, Any]:
        """Mock distance matrix"""
        return self._get_response("get_distance_matrix", {
            "rows": [
                {
                    "elements": [
                        {
                            "distance": {"value": random.randint(1000, 10000), "text": f"{random.randint(1, 10)} km"},
                            "duration": {"value": random.randint(300, 1800), "text": f"{random.randint(5, 30)} mins"},
                            "status": "OK"
    ...
    async def geocode_address(self, address: str) -> Dict[str, Any]:
        """Mock geocoding"""
        return self._get_response("geocode_address", {
            "lat": 25.033 + random.uniform(-0.1, 0.1),
            "lng": 121.565 + random.uniform(-0.1, 0.1),
            "formatted_address": address,
            "place_id": "place_" + ''.join(random.choices(string.ascii_letters, k=15))
        })
...
class MockVertexAIService(MockService):
    ...
    async def predict_order_volume(self, customer_ids: List[int], **kwargs) -> Dict[str, Any]:
        ...
        return self._get_response("predict_order_volume", {
    ...
    async def analyze_customer_behavior(self, customer_id: int) -> Dict[str, Any]:
        ...
        return self._get_response("analyze_customer_behavior", {
            "customer_id": customer_id,
            "churn_risk": round(random.uniform(0, 0.3), 2),
            "lifetime_value": random.randint(50000, 500000),
            "preferred_delivery_days": random.sample(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"], 2),
...
class MockSMSService(MockService):
    ...
    async def send_sms(self, phone: str, message: str) -> Dict[str, Any]:
        """Mock SMS sending"""
        return self._get_response("send_sms", {
    ...
    async def send_bulk_sms(self, recipients: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    async def get_sms_status(self, message_id: str) -> Dict[str, Any]:
        ...
        return self._get_response("get_sms_status", {
            "message_id": message_id,
            "status": random.choice(["delivered", "pending", "failed"]),
            "delivered_at": datetime.now().isoformat() if random.random() > 0.2 else None,
...
class MockEInvoiceService(MockService):
    ...
    async def issue_invoice(self, invoice_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self._get_response("issue_invoice", {
            "invoice_number": "AA" + ''.join(random.choices(string.digits, k=8)),
            "status": "issued",
            "issue_date": datetime.now().isoformat(),
            "qr_code": "https://einvoice.nat.gov.tw/qrcode/" + ''.join(random.choices(string.ascii_letters, k=20)),
            "verification_code": ''.join(random.choices(string.digits, k=4))
        })
    
    async def void_invoice(self, invoice_number: str, reason: str) -> Dict[str, Any]:
        """Mock invoice voiding"""
        return self._get_response("void_invoice", {
            "invoice_number": invoice_number,
            "status": "voided",
            "void_date": datetime.now().isoformat(),
            "void_reason": reason,
            "confirmation_number": "VOID" + ''.join(random.choices(string.digits, k=6))
        })
