...
class TestBankingService:
            ...
            with patch("paramiko.RSAKey.from_private_key") as mock_key_class:
                ...
                with patch("paramiko.SFTPClient.from_transport") as mock_sftp_class:
                    ...
                    with banking_service.get_sftp_client(bank_config) as sftp:
                        assert sftp == mock_sftp
                        mock_transport.connect.assert_called_once_with(
                            username=bank_config.sftp_username, pkey=mock_key
                        )
    ...
    def test_generate_fixed_width_payment_file(
        self,
        banking_service,
        payment_batch,
    ...
    def test_generate_csv_payment_file(
        self,
        banking_service,
        payment_batch,
        payment_transactions,
    ...
    def test_upload_payment_file_success(
        self, banking_service, payment_batch, bank_config, db_session
    ):
    ...
    def test_upload_payment_file_failure(
        self, banking_service, payment_batch, bank_config, db_session
    ):
...
T00000200000300000"""
        ...
        log = ReconciliationLog(
            file_name="REC_20240120.txt", file_received_at=datetime.utcnow()
        )
        ...
        with patch.object(banking_service, "get_sftp_client") as mock_get_sftp:
            ...
            result = banking_service.process_reconciliation_file(
                bank_config.bank_code, "REC_20240120.txt"
            )
    ...
    def test_create_payment_batch(self, banking_service, db_session):
        ...
        db_session.query().filter_by().scalar.return_value = (
            "fixed_width"  # File format
        )
        ...
        batch = banking_service.create_payment_batch(
            bank_code="CTBC", processing_date=datetime(2024, 1, 20)
        )
    ...
    def test_get_payment_status_report(
        self, banking_service, payment_batch, db_session
    ):
