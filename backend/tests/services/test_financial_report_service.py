...
class TestFinancialReportService:
    ...
    async def test_get_revenue_summary(self, report_service, sample_invoices):
        ...
        mock_result.scalars = Mock(
            return_value=Mock(all=Mock(return_value=sample_invoices))
        )
    ...
    async def test_get_accounts_receivable_aging(self, report_service):
        ...
        mock_result = Mock()
        mock_result.scalars = Mock(
            return_value=Mock(all=Mock(return_value=unpaid_invoices))
        )
        report_service.db.execute = AsyncMock(return_value=mock_result)
        ...
        assert aging_report["total_receivable"] == Decimal(
            "55000"
        )  # 10000 + 15000 + 30000
    ...
    async def test_get_customer_statement(self, report_service):
        ...
        mock_invoice_result.scalars = Mock(
            return_value=Mock(all=Mock(return_value=invoices))
        )

        report_service.db.execute = AsyncMock(
            side_effect=[mock_customer_result, mock_invoice_result]
        )
    ...
    async def test_generate_401_file(self, report_service):
        ...
        mock_result = Mock()
        mock_result.scalars = Mock(
            return_value=Mock(all=Mock(return_value=b2b_invoices))
        )
        report_service.db.execute = AsyncMock(return_value=mock_result)
