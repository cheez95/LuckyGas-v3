...
from app.models import (
    User, Customer, GasProduct, Order, OrderItem,
    Route, RouteDelivery, Vehicle, DeliveryHistory,
    DeliveryHistoryItem, Invoice, Notification,
    OrderTemplate, CustomerInventory
)
...
TAIWAN_STREETS = ["中山路", "中正路", "民生路", "民權路", "復興路", "和平路", "信義路", "仁愛路", "忠孝路", "光復路", "建國路", "南京路"]
...
class TestDataGenerator:
    ...
    async def clean_database(self):
        ...
        tables_to_clean = [
            DeliveryHistoryItem, DeliveryHistory, Invoice,
            Notification, RouteDelivery, Route, OrderItem,
            Order, OrderTemplate, CustomerInventory, Customer,
            Vehicle, GasProduct, User
        ]
        ...
        for customer in self.customers[:30]:  # First 30 customers have inventory
            for product in self.products[:3]:  # Gas products only
                inventory = CustomerInventory(
                    ...
                    last_delivery_date=datetime.now() - timedelta(days=random.randint(7, 30))
                )
    ...
    async def create_routes(self):
        ...
        pending_orders = [o for o in self.orders if o.status in ["pending", "confirmed"]]
        ...
        for day_offset in range(7):
            ...
            for route_num in range(num_routes):
                driver = random.choice([u for u in self.users if u.role == UserRole.DRIVER])
                ...
                route_orders = random.sample(
                    ...
                    min(random.randint(5, 15), len([o for o in pending_orders if o.delivery_date.date() == route_date]))
                )
    ...
    async def create_delivery_history(self):
        ...
        for order in completed_orders[:50]:  # First 50 completed orders
            delivery = DeliveryHistory(
                ...
                delivered_by=random.choice([u.id for u in self.users if u.role == UserRole.DRIVER]),
                signature_image=None,
                delivery_photo=None,
                notes=f"已成功送達 - {order.customer_name}",
                delivery_latitude=order.delivery_latitude + random.uniform(-0.001, 0.001),
                delivery_longitude=order.delivery_longitude + random.uniform(-0.001, 0.001),
                delivery_time=order.delivery_date + timedelta(hours=random.randint(8, 18))
            )
            ...
            for item in order_items:
                delivery_item = DeliveryHistoryItem(
                    ...
                    quantity_empty_returned=random.randint(0, item.quantity) if random.random() > 0.5 else 0
                )
