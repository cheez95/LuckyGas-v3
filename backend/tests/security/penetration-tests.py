...
class SecurityPenetrationTest:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for payload in sql_payloads:
                ...
                if response.status_code == 200:
                    ...
                    result["vulnerabilities"].append(f"Login endpoint vulnerable to: {payload}")
            ...
            for endpoint in search_endpoints:
                for payload in sql_payloads:
                    ...
                    if "error" in response.text.lower() and "sql" in response.text.lower():
                        result["status"] = "VULNERABLE"
                        result["vulnerabilities"].append(f"{endpoint} exposes SQL errors")
        ...
        async with httpx.AsyncClient() as client:
            for payload in nosql_payloads:
                try:
                    response = await client.post(
                        f"{self.base_url}/api/v1/auth/login",
                        json=payload if isinstance(payload, dict) else {"username": payload, "password": "test"}
                    )
                    
                    if response.status_code == 200:
                        result["status"] = "VULNERABLE"
                        result["vulnerabilities"].append(f"NoSQL injection with: {payload}")
                except:
                    pass
        ...
        async with httpx.AsyncClient() as client:
            ...
            for payload in cmd_payloads:
                ...
                if response.status_code == 200:
                    ...
                    if "uid=" in response.text or "root" in response.text:
                        result["status"] = "VULNERABLE"
                        result["vulnerabilities"].append(f"Command injection in file upload: {payload}")
            ...
            for params in export_params:
                ...
                if "command not found" not in response.text.lower():
                    if any(indicator in response.text for indicator in ["uid=", "root", "Directory"]):
                        result["status"] = "VULNERABLE"
                        result["vulnerabilities"].append("Command injection in export functionality")
        ...
        async with httpx.AsyncClient() as client:
            for password in weak_passwords:
                ...
                if response.status_code == 201:
                    result["status"] = "VULNERABLE"
                    result["vulnerabilities"].append(f"Accepted weak password: {password}")
    ...
    async def test_brute_force_protection(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if failed_attempts >= 10:
                result["status"] = "VULNERABLE"
                result["vulnerabilities"].append(f"No rate limiting after {failed_attempts} failed attempts")
            ...
            try:
                ...
                for secret in weak_secrets:
                    try:
                        ...
                        if response.status_code == 200:
                            result["status"] = "VULNERABLE"
                            result["vulnerabilities"].append(f"Weak JWT secret: {secret}")
                            break
                    except:
                        pass
                ...
                expired_token = jwt.encode(expired_payload, "some_secret", algorithm="HS256")
            ...
            if response.status_code == 201:
                ...
                for offset in [-5, -1, 1, 5, 10]:
                    ...
                    if other_id > 0 and other_id != my_order_id:
                        ...
                        if response.status_code == 200:
                            result["status"] = "VULNERABLE"
                            result["vulnerabilities"].append(f"Can access order {other_id} without authorization")
                ...
                if isinstance(my_order_id, str) and "-" in my_order_id:
                    ...
                    try:
                        ...
                        for i in range(1, 5):
                            ...
                            if response.status_code == 200:
                                result["status"] = "VULNERABLE"
                                result["vulnerabilities"].append("Predictable UUID generation")
                                break
                    except:
                        pass
            ...
            for origin in malicious_origins:
                ...
                if allow_origin == origin or allow_origin == "*":
                    if allow_credentials == "true":
                        result["status"] = "VULNERABLE"
                        result["vulnerabilities"].append(f"Allows credentials from origin: {origin}")
                    elif allow_origin == "*":
                        result["status"] = "VULNERABLE"
                        result["vulnerabilities"].append("Wildcard origin with credentials")
    ...
    async def test_security_headers(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for header, expected_values in required_headers.items():
                ...
                if not header_value:
                    result["status"] = "VULNERABLE"
                    result["vulnerabilities"].append(f"Missing security header: {header}")
                else:
                    if isinstance(expected_values, list):
                        if not any(expected in header_value for expected in expected_values):
                            result["status"] = "VULNERABLE"
                            result["vulnerabilities"].append(f"Weak {header}: {header_value}")
                    ...
                    if response.status_code == 200:
                        ...
                        if any(payload in str(value) for value in data.values()):
                            ...
                            if payload in html_response.text:
                                result["status"] = "VULNERABLE"
                                result["vulnerabilities"].append("Stored XSS in customer data")
    ...
    async def test_security_event_logging(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            suspicious_activities = [
                ...
                ("POST", "/api/v1/auth/login", {"username": "admin", "password": "wrong"}, "Failed login"),
                # SQL injection attempt
                ("GET", "/api/v1/customers/search?q=' OR '1'='1", None, "SQL injection attempt"),
                # Path traversal
            ...
            admin_headers = {"Authorization": f"Bearer {self.valid_token}"}  # Assume admin token
            ...
            if response.status_code == 404:
                ...
                missing_logs = set([a[3] for a in suspicious_activities]) - set(logged_activities)
                if missing_logs:
                    result["status"] = "VULNERABLE"
                    result["vulnerabilities"].append(f"Not logging: {', '.join(missing_logs)}")
            ...
            for endpoint in version_endpoints:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        vulnerable_versions = {
                            "fastapi": ["0.1.0", "0.2.0"],  # Example vulnerable versions
                            ...
                            if lib in str(data).lower():
                                for version in vulnerable:
                                    if version in str(data):
                                        result["status"] = "VULNERABLE"
                                        result["vulnerabilities"].append(f"{lib} {version} has known vulnerabilities")
                    except:
                        pass
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        for vuln_type, recommendation in vuln_recommendations.items():
            if any(vuln_type in test["test"] for cat in self.results for test in cat["tests"] if test["status"] == "VULNERABLE"):
                report["recommendations"].append({
                    "priority": "MEDIUM",
