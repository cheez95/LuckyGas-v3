...
async def test_database():
    ...
    test_db_url = settings.DATABASE_URL.replace('luckygas_db', 'luckygas_migration_test')
    ...
    async with engine.begin() as conn:
        ...
        # Create indexes
        await conn.execute(text("CREATE INDEX idx_customers_active ON customers(is_active)"))
        await conn.execute(text("CREATE INDEX idx_customers_corporate ON customers(is_corporate)"))
        await conn.execute(text("CREATE INDEX idx_customers_area ON customers(delivery_area)"))
...
def mock_excel_file(tmp_path):
    ...
    data = {
        ...
        '地址': ['台北市信義區信義路1號', '台北市大安區忠孝東路2號', '台北市中山區中山北路3號'],
        '系統上鋼瓶數量': [10, 20, 30],
        '50KG': [2, 4, 6],
        '20KG': [3, 5, 7],
        '16KG': [1, 2, 3],
