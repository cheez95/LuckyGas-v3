...
class MigrationTestRunner:
        ...
        for test_file, suite_name in test_suites:
            ...
            if os.path.exists(file_path):
                ...
                if 'summary' in results:
                    self.test_results['summary']['total_tests'] += results['summary'].get('total', 0)
                    self.test_results['summary']['passed'] += results['summary'].get('passed', 0)
                    self.test_results['summary']['failed'] += results['summary'].get('failed', 0)
                    self.test_results['summary']['skipped'] += results['summary'].get('skipped', 0)
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} Taiwan Time\n")
            ...
            for suite_name, suite_data in self.test_results['test_suites'].items():
                ...
                f.write(f"- **Status**: {'PASSED' if suite_data['exit_code'] == 0 else 'FAILED'}\n\n")
                
                if suite_data.get('tests'):
                    f.write("**Test Cases**:\n\n")
                    for test in suite_data['tests']:
                        status_icon = '✅' if test.get('outcome') == 'passed' else '❌'
                        f.write(f"- {status_icon} `{test.get('nodeid', 'Unknown test')}`\n")
                    f.write("\n")
            ...
            if 'validation_script' in self.test_results:
                ...
                f.write(f"- **Status**: {'PASSED' if val_result['success'] else 'FAILED'}\n")
            ...
            if summary['failed'] == 0:
                ...
                f.write("All tests passed successfully. The migration script is ready for production use.\n\n")
                ...
                f.write(f"{summary['failed']} tests failed. Please address the following:\n\n")
                
                # List failed tests
                for suite_name, suite_data in self.test_results['test_suites'].items():
                    if suite_data.get('tests'):
                        failed_tests = [t for t in suite_data['tests'] if t.get('outcome') != 'passed']
                        if failed_tests:
                            f.write(f"**{suite_name}**:\n")
                            for test in failed_tests:
                                f.write(f"- Fix: `{test.get('nodeid', 'Unknown test')}`\n")
                            f.write("\n")
            ...
            f.write("---\n\n")
            f.write("**Report Generated**: " + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + " Taiwan Time\n")
