    ...
    for endpoint in config.get("test_endpoints", []):
        try:
            ...
            if method == "get":
                ...
                results.append((endpoint['path'], False, f"Unsupported method: {method}"))
                continue
            
            if response.status_code in [200, 301, 302]:  # Allow redirects for docs
                results.append((endpoint['path'], True, f"Status: {response.status_code}"))
            else:
                results.append((endpoint['path'], False, f"Status: {response.status_code}"))
...
def print_service_status(service_name: str, health_ok: bool, health_msg: str, endpoint_results: List):
...
def print_summary(results: Dict[str, Tuple[bool, str, List]]):
    ...
    if healthy_services == total_services:
        ...
        print("  1. Check if Docker containers are running: docker-compose -f docker-compose.test.yml ps")
        print("  2. Check container logs: docker-compose -f docker-compose.test.yml logs [service-name]")
        print("  3. Restart services: docker-compose -f docker-compose.test.yml restart")
        print("  4. Rebuild services: docker-compose -f docker-compose.test.yml build --no-cache")
        return False
