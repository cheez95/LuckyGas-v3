...
class TestResourceExhaustion:
    ...
    async def test_memory_exhaustion_handling(self, client: AsyncClient):
        ...
        try:
            ...
            for i in range(max_blocks):
                try:
                    ...
                    if response_time > 2.0 or process.memory_percent() > 80:
                        print(f"Stopping at block {i}, memory {memory_increase:.1f}MB, "
            ...
            if allocation_results:
                ...
                early_response_times = [r["api_response_time"] for r in allocation_results[:5]]
                late_response_times = [r["api_response_time"] for r in allocation_results[-5:]]
                
                avg_early = sum(early_response_times) / len(early_response_times)
                avg_late = sum(late_response_times) / len(late_response_times) if late_response_times else avg_early
                ...
                failed_requests = [r for r in allocation_results if r["api_status"] != 200]
                failure_rate = len(failed_requests) / len(allocation_results)
                assert failure_rate < 0.2, f"Too many failures under memory pressure: {failure_rate:.1%}"
    ...
    async def test_cpu_throttling_behavior(self, client: AsyncClient):
        ...
        # Analyze results
        successful_requests = [m for m in performance_metrics if m["status_code"] == 200]
        ...
        print(f"Average CPU during test: {sum(m['cpu_percent'] for m in performance_metrics) / len(performance_metrics):.1f}%")
        
        # System should maintain reasonable performance
        assert success_rate >= 0.8, f"Success rate too low under CPU load: {success_rate:.1%}"
        assert avg_response_time < baseline_time * 5, \
    ...
    async def test_disk_space_exhaustion(self, client: AsyncClient):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                chunks_written = 0
                max_chunks = min(int(free_space_mb / 10), 50)  # Limit to 500MB or available space
                ...
                if disk_fill_results:
                    ...
                    early_results = disk_fill_results[:5] if len(disk_fill_results) >= 5 else disk_fill_results
                    late_results = disk_fill_results[-5:] if len(disk_fill_results) >= 5 else disk_fill_results
                    
                    # Logging should degrade gracefully
                    early_log_success = sum(1 for r in early_results if r["log_status"] in [200, 201]) / len(early_results)
                    late_log_success = sum(1 for r in late_results if r["log_status"] in [200, 201]) / len(late_results)
                    ...
                    assert late_log_success >= 0.5 or late_results[-1]["disk_percent"] > 95, \
                    ...
                    if late_results[-1]["disk_percent"] > 90:
                        late_upload_failures = sum(1 for r in late_results if r["upload_status"] >= 500)
                        assert late_upload_failures < len(late_results), \
    ...
    async def test_file_descriptor_exhaustion(self, client: AsyncClient):
        ...
        try:
            ...
            if fd_exhaustion_results:
                ...
                failed_health_checks = sum(1 for r in fd_exhaustion_results if r["health_status"] != 200)
                assert failed_health_checks == 0, \
                    "Health checks should not fail due to FD exhaustion"
                
                # Database queries might fail, but should not crash
                db_errors = sum(1 for r in fd_exhaustion_results if r["db_query_status"] >= 500)
                assert db_errors < len(fd_exhaustion_results) / 2, \
    ...
    async def test_thread_exhaustion(self, client: AsyncClient):
        ...
        try:
            ...
            for i in range(max_threads):
                try:
                    ...
                    if i % 20 == 0:
                        ...
                        print(f"Threads: {current_thread_count}, Response time: {response_time:.3f}s")
            ...
            while threading.active_count() > initial_thread_count and (time.time() - start_wait) < max_wait:
    ...
    async def test_connection_pool_exhaustion(self, client: AsyncClient):
        ...
        async def make_request(request_id):
            ...
            try:
                ...
                response = await client.get(
                    "/api/v1/customers",
                    headers={"Authorization": "Bearer test-token", "Connection": "keep-alive"}
                )
                return {
        ...
        assert slow_requests < concurrent_count * 0.2, \
