...
class TestDatabaseChaos:
    ...
    async def test_connection_pool_exhaustion(self, client: AsyncClient, test_settings):
        ...
        assert successful_connections >= 2, "At least pool_size connections should succeed"
    ...
    async def test_database_connection_failure_recovery(self, client: AsyncClient):
        ...
        with patch('asyncpg.connect', side_effect=OperationalError("Database unavailable", "", "")):
        ...
        assert not any(outage_health), "Database incorrectly reported as healthy during outage"
        ...
        assert any(r["healthy"] for r in recovery_checks), \
            "Database did not recover after connection restored"
    
    @pytest.mark.chaos
    @pytest.mark.asyncio
    async def test_slow_query_handling(self, client: AsyncClient, db_session: AsyncSession):
    ...
    @pytest.mark.chaos
    @pytest.mark.asyncio
    async def test_transaction_rollback_integrity(self, client: AsyncClient, db_session: AsyncSession):
        ...
        with patch('app.services.order_service.OrderService.calculate_totals', 
    ...
    async def test_connection_leak_prevention(self, client: AsyncClient):
        ...
        assert connection_increase < 10, \
            f"Possible connection leak detected: {connection_increase} new connections"
    
    @pytest.mark.chaos
    @pytest.mark.asyncio
    async def test_deadlock_detection_and_recovery(self, client: AsyncClient, db_session: AsyncSession):
        ...
        results = await asyncio.gather(
            transaction_a(db_session),
            transaction_b(db_session),
        ...
        assert successful >= 1, "Both transactions failed - deadlock not handled properly"
    ...
    async def test_database_failover(self, client: AsyncClient):
        ...
        with patch('app.core.database.get_async_session') as mock_session:
            ...
            while (time.time() - start_time) < failover_duration:
                ...
                if elapsed < 3:
                    ...
                    mock_session.side_effect = OperationalError("Primary database down", "", "")
                elif elapsed < 5:
                    # Failover in progress - intermittent failures
                    if int(elapsed * 10) % 2 == 0:
                        mock_session.side_effect = OperationalError("Failover in progress", "", "")
                    else:
        ...
        available_checks = sum(1 for check in availability_timeline if check["available"])
        ...
        assert availability_percentage >= 50, \
