...
class TestPodFailureRecovery:
    ...
    async def test_single_pod_termination(self, client: AsyncClient):
        ...
        assert (
            service_available
        ), f"Service did not recover after {recovery_attempts} seconds"
    ...
    async def test_rolling_restart_availability(self, client: AsyncClient):
        ...
        print(
            f"Service availability during rolling restart: {availability_percentage:.2f}%"
        )
    ...
    @pytest.mark.chaos
    @pytest.mark.asyncio
    async def test_graceful_shutdown_data_integrity(
    ...
    async def test_cascading_failure_prevention(self, client: AsyncClient):
        ...
        healthy_components = sum(
            1 for status in component_status.values() if status["available"]
        )
