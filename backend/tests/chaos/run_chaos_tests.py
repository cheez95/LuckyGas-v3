...
class ChaosTestRunner:
    ...
    def run_all_suites(self) -> None:
        ...
        suites = [
            ("pod_failure", ["test_single_pod_termination", "test_rolling_restart_availability"]),
            ("network_chaos", ["test_network_latency_100ms", "test_network_latency_500ms", "test_intermittent_network_failures"]),
            ("database_chaos", ["test_connection_pool_exhaustion", "test_database_connection_failure_recovery"]),
            ("external_api_chaos", ["test_einvoice_api_failure", "test_sms_gateway_timeout", "test_multiple_external_api_failures"]),
            ("resource_exhaustion", ["test_memory_exhaustion_handling", "test_cpu_throttling_behavior"])
        ]
        ...
        for suite_name, priority_tests in suites:
            ...
            print(f"{status_emoji} {suite_name}: {results['status']} ({results['duration']:.2f}s)")
            
            # If critical suite fails, optionally stop
            if suite_name in ["pod_failure", "database_chaos"] and results["status"] != "passed":
                print(f"⚠️  Critical suite {suite_name} failed. Continuing with remaining tests...")
    ...
    def generate_report(self) -> None:
        ...
        passed_tests = sum(1 for r in self.results["tests"].values() if r["status"] == "passed")
        failed_tests = sum(1 for r in self.results["tests"].values() if r["status"] == "failed")
        error_tests = sum(1 for r in self.results["tests"].values() if r["status"] == "error")
        ...
        self.results["summary"] = {
            ...
            "total_duration": sum(r.get("duration", 0) for r in self.results["tests"].values())
        }
    ...
    def generate_markdown_report(self) -> None:
        ...
        with open(md_path, 'w') as f:
            ...
            f.write("## Summary\n\n")
            f.write(f"- **Total Test Suites**: {self.results['summary']['total_suites']}\n")
            ...
            for suite_name, results in self.results["tests"].items():
                ...
                if "detailed_results" in results and "tests" in results["detailed_results"]:
                    f.write(f"- **Test Count**: {len(results['detailed_results']['tests'])}\n")
                    
                    # Show failed tests
                    failed_tests = [t for t in results["detailed_results"]["tests"] 
            ...
            if self.results["summary"]["success_rate"] < 80:
                ...
                f.write("- Consider implementing additional fault tolerance mechanisms\n")
