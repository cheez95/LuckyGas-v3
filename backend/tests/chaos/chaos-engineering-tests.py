...
class ChaosTest:
    def __init__(
        self, base_url: str = "http://localhost:8000", k8s_namespace: str = "luckygas"
    ):
    ...
    async def test_pod_failure_recovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pods = self.v1.list_namespaced_pod(
                namespace=self.k8s_namespace, label_selector="app=backend"
            )
            ...
            self.v1.delete_namespaced_pod(name=pod_name, namespace=self.k8s_namespace)
    ...
    async def test_network_partition(self) -> Dict[str, Any]:
        ...
        try:
            ...
            networking_v1.create_namespaced_network_policy(
                namespace=self.k8s_namespace, body=network_policy
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                if "網路連線中斷" in response.text or response.status_code == 503:
                    test_result["details"].append(
                        "Frontend handles network partition gracefully"
                    )
                else:
                    test_result["status"] = "FAILED"
                    test_result["details"].append(
                        "Frontend doesn't handle network partition"
                    )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if health_data.get("database") == "disconnected":
                    test_result["details"].append(
                        "Health check correctly reports database issue"
                    )
                ...
                if response.status_code in [503, 500]:
                    test_result["details"].append(
                        "API returns appropriate error for database failure"
                    )
                else:
                    test_result["status"] = "FAILED"
                    test_result["details"].append(
                        "API doesn't handle database failure properly"
                    )
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if health_data.get("database") == "connected":
                            recovery_time = time.time() - recovery_start
                            test_result["details"].append(
                                f"Database recovered in {recovery_time:.2f}s"
                            )
                            break
                await asyncio.sleep(5)
    ...
    async def test_external_api_timeout(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    test_result["details"].append(
                        f"Route optimization handled slow API ({response_time:.2f}s)"
                    )
                elif response.status_code == 408:
                    test_result["details"].append(
                        "API correctly times out slow external calls"
                    )
                else:
                    test_result["status"] = "FAILED"
                    test_result["details"].append(
    ...
    async def test_resource_exhaustion(self) -> Dict[str, Any]:
        ...
        test_result = {"test": "Resource Exhaustion", "status": "PASSED", "details": []}
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                test_result["details"].append(
                    f"Success: {success_count}, Rate limited: {rate_limited}, Errors: {errors}"
                )
    ...
    async def test_zone_failure_simulation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recovery_time:
                test_result["details"].append(
                    f"Service recovered in {recovery_time:.2f}s after zone failure"
                )
                ...
                test_result["details"].append(
                    "Service maintained availability during zone failure"
                )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if login_response.status_code == 200:
                    test_result["details"].append(
                        "Auth service works without Redis (with limitations)"
                    )
            ...
            test_result["details"].append(
                f"Success rate during partial failure: {success_rate:.2%}"
            )
            ...
            if success_rate > 0.5:  # At least 50% success
                ...
                test_result["status"] = "FAILED"
                test_result["details"].append(
                    "System failed to maintain minimum functionality"
                )
