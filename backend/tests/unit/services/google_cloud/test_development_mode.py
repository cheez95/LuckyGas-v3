...
class TestDevelopmentModeManager:
    ...
    def test_is_production_mode(self, reset_env):
        ...
        with patch.dict(os.environ, {"GOOGLE_API_MODE": "production", "TESTING": "false"}, clear=True):
            manager = DevelopmentModeManager()
            assert manager.is_production() is True
        
        # Test development mode
        with patch.dict(os.environ, {"GOOGLE_API_MODE": "development", "TESTING": "false"}, clear=True):
            manager = DevelopmentModeManager()
    ...
    def test_is_offline_mode(self, reset_env):
        ...
        # Test offline mode
        with patch.dict(os.environ, {"GOOGLE_API_MODE": "offline", "TESTING": "false"}, clear=True):
            manager = DevelopmentModeManager()
