...
class TestGoogleAPIRateLimiter:
    ...
    def mock_redis(self):
        ...
        with patch("app.services.google_cloud.monitoring.rate_limiter.get_redis_client", AsyncMock(return_value=mock_client)):
            yield mock_client
    ...
    async def test_different_api_types(self, rate_limiter, mock_redis):
        ...
        allowed_vertex, wait_time_vertex = await rate_limiter.check_rate_limit("vertex_ai")
    ...
    async def test_reset_limits(self, rate_limiter, mock_redis):
        ...
        assert mock_redis.delete.call_count == 3  # Should be called 3 times for each window
    ...
    async def test_concurrent_requests(self, rate_limiter, mock_redis):
        ...
        # Verify
        assert all(allowed for allowed, _ in results)  # All should be allowed (under limit)
