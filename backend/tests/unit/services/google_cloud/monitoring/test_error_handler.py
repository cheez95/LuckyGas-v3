...
class TestGoogleAPIErrorHandler:
    ...
    def test_classify_error_by_status_code(self):
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=400)
            == APIErrorType.INVALID_REQUEST
        )
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=401)
            == APIErrorType.INVALID_API_KEY
        )
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=403)
            == APIErrorType.PERMISSION_DENIED
        )
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=404)
            == APIErrorType.NOT_FOUND
        )
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=429)
            == APIErrorType.RATE_LIMIT
        )
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=500)
            == APIErrorType.INTERNAL_ERROR
        )
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=503)
            == APIErrorType.SERVICE_UNAVAILABLE
        )

        # Test unknown status code
        assert (
            GoogleAPIErrorHandler.classify_error(status_code=418)
    ...
    def test_classify_error_by_exception(self):
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(exception=timeout_error)
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(exception=client_error)
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(exception=conn_error)
            == APIErrorType.NETWORK_ERROR
        )
    ...
    def test_classify_error_by_response_body(self):
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(error_body=quota_body)
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(error_body=rate_body)
            == APIErrorType.RATE_LIMIT
        )
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(error_body=key_body)
            == APIErrorType.INVALID_API_KEY
        )
        ...
        assert (
            GoogleAPIErrorHandler.classify_error(error_body=perm_body)
            == APIErrorType.PERMISSION_DENIED
        )
    ...
    def test_should_retry(self):
        ...
        assert (
            GoogleAPIErrorHandler.should_retry(APIErrorType.SERVICE_UNAVAILABLE) is True
        )
        ...
        assert (
            GoogleAPIErrorHandler.should_retry(APIErrorType.PERMISSION_DENIED) is False
        )
    ...
    def test_get_retry_delay(self):
        ...
        delay = GoogleAPIErrorHandler.get_retry_delay(
            APIErrorType.RATE_LIMIT, attempt=0
        )
        assert 0.8 <= delay <= 1.2  # 1.0 base with ±20% jitter

        delay = GoogleAPIErrorHandler.get_retry_delay(
            APIErrorType.RATE_LIMIT, attempt=1
        )
        ...
        delay = GoogleAPIErrorHandler.get_retry_delay(
            APIErrorType.SERVICE_UNAVAILABLE, attempt=0
        )
    ...
    async def test_handle_with_retry_success(self):
        ...
        result = await GoogleAPIErrorHandler.handle_with_retry(
            successful_func, api_type="test", endpoint="test_endpoint"
        )
    ...
    async def test_handle_with_retry_transient_error(self):
        ...
        with patch("asyncio.sleep"):
            # Execute
            result = await GoogleAPIErrorHandler.handle_with_retry(
                flaky_func, api_type="test", endpoint="test_endpoint"
            )
    ...
    async def test_handle_with_retry_non_retryable(self):
        ...
        async def perm_error_func():
            ...
            error = aiohttp.ClientResponseError(
                request_info=Mock(), history=(), status=403, message="Permission Denied"
            )
            error.headers = {}  # Add empty headers
            raise error
        ...
        with pytest.raises(GoogleAPIError) as exc_info:
            await GoogleAPIErrorHandler.handle_with_retry(
                perm_error_func, api_type="test", endpoint="test_endpoint"
            )
    ...
    async def test_handle_with_retry_max_retries(self):
        ...
        with patch("asyncio.sleep"):
            ...
            with pytest.raises(GoogleAPIError) as exc_info:
                await GoogleAPIErrorHandler.handle_with_retry(
                    always_fails, api_type="test", endpoint="test_endpoint"
                )
    ...
    async def test_handle_with_retry_network_error(self):
        ...
        with patch("asyncio.sleep"):
            # Execute
            result = await GoogleAPIErrorHandler.handle_with_retry(
                network_error_func, api_type="test", endpoint="test_endpoint"
            )
    ...
    async def test_handle_with_retry_timeout_error(self):
        ...
        with patch("asyncio.sleep"):
            # Execute
            result = await GoogleAPIErrorHandler.handle_with_retry(
                timeout_func, api_type="test", endpoint="test_endpoint"
            )
    ...
    async def test_handle_with_retry_unexpected_error(self):
        ...
        with pytest.raises(GoogleAPIError) as exc_info:
            await GoogleAPIErrorHandler.handle_with_retry(
                unexpected_error_func, api_type="test", endpoint="test_endpoint"
            )
