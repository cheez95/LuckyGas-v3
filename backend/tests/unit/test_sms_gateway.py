...
def mock_twilio_provider():
    ...
    provider.check_status = AsyncMock(return_value={
        "success": True,
...
class TestSMSGateway:
    ...
    async def test_send_sms_primary_provider(self, sms_gateway, mock_twilio_provider):
        ...
        result = await sms_gateway.send_sms(
            phone="0912345678",
            message="測試簡訊"
        )
    ...
    @pytest.mark.asyncio
    async def test_send_sms_fallback_provider(self, sms_gateway, mock_twilio_provider, mock_chunghwa_provider):
    ...
    @pytest.mark.asyncio
    async def test_health_checks(self, sms_gateway, mock_twilio_provider, mock_chunghwa_provider):
    ...
    @pytest.mark.asyncio
    async def test_all_providers_fail(self, sms_gateway, mock_twilio_provider, mock_chunghwa_provider):
