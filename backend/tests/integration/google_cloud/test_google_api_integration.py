...
class TestGoogleAPIIntegration:
    ...
    async def test_rate_limiting_across_services(self, routes_service, vertex_service):
        ...
        routes_allowed, routes_wait = await routes_service._rate_limiter.check_rate_limit("routes")
        vertex_allowed, vertex_wait = await vertex_service.rate_limiter.check_rate_limit("vertex_ai")
    ...
    async def test_cost_monitoring_aggregation(self, routes_service, vertex_service):
        ...
        routes_service._cost_monitor.redis.incrbyfloat = AsyncMock(
            return_value=10.50
        )
        ...
        vertex_service.cost_monitor.redis.incrbyfloat = AsyncMock(
            return_value=5.25
        )
        ...
        await routes_service._cost_monitor.record_api_call(
            "routes", "optimization"
        )
        await vertex_service.cost_monitor.record_api_call(
            "vertex_ai", "prediction"
        )
    ...
    async def test_cache_sharing_prevention(self, routes_service, vertex_service):
        ...
        assert routes_key.startswith("google_api:routes:"), f"Unexpected routes key: {routes_key}"
        assert vertex_key.startswith("google_api:vertex_ai:"), f"Unexpected vertex key: {vertex_key}"
    ...
    async def test_error_handling_cascade(self, routes_service):
        ...
        # Mock checks
        routes_service._rate_limiter.check_rate_limit = AsyncMock(return_value=(True, 0))
        ...
        routes_service._rate_limiter.check_rate_limit = AsyncMock(return_value=(False, 60))
        
        allowed, wait_time = await routes_service._rate_limiter.check_rate_limit("routes")
        assert allowed is False
        assert wait_time == 60
        
        # Test budget limit error handling
        routes_service._rate_limiter.check_rate_limit = AsyncMock(return_value=(True, 0))
        routes_service._cost_monitor.enforce_budget_limit = AsyncMock(return_value=False)
    ...
    async def test_health_check_aggregation(self, routes_service, vertex_service):
        ...
        routes_rate_ok, _ = await routes_service._rate_limiter.check_rate_limit("routes")
        assert routes_rate_ok == True
        
        routes_budget_ok = await routes_service._cost_monitor.enforce_budget_limit("routes")
        assert routes_budget_ok == True
        
        # Vertex service should be unhealthy
        vertex_rate_ok, wait_time = await vertex_service.rate_limiter.check_rate_limit("vertex_ai")
        assert vertex_rate_ok == False
        assert wait_time == 60
        
        vertex_budget_ok = await vertex_service.cost_monitor.enforce_budget_limit("vertex_ai")
    ...
    async def test_concurrent_service_requests(self, routes_service, vertex_service):
        ...
        with patch.dict(os.environ, {"DEVELOPMENT_MODE": "development"}):
            ...
            async def mock_predict():
                return {
                    "predictions": [{"customer_id": i, "probability": 0.8} for i in range(5)],
                    "batch_id": "test_batch"
                }
