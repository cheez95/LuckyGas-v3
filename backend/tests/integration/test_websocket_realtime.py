...
class TestWebSocketRealTimeUpdates:
    ...
    async def test_websocket_connection_lifecycle(self):
        ...
        await websocket_manager.send_personal_message(
            json.dumps(test_message), connection_id
        )
        ...
        assert connection_id not in websocket_manager.user_connections.get(
            user_id, set()
        )
    ...
    async def test_role_based_broadcasting(self):
        ...
        await websocket_manager.broadcast_to_role(
            json.dumps(driver_message), UserRole.DRIVER.value
        )
    ...
    async def test_order_status_update_broadcast(self, db_session, test_customer):
        ...
        await websocket_manager.broadcast_to_role(
            json.dumps(order_update), UserRole.OFFICE_STAFF.value
        )
        await websocket_manager.send_to_user(json.dumps(order_update), test_customer.id)
    ...
    async def test_driver_location_updates(self):
        ...
        await websocket_manager.broadcast_to_role(
            json.dumps(location_update), UserRole.OFFICE_STAFF.value
        )
        await websocket_manager.broadcast_to_role(
            json.dumps(location_update), UserRole.MANAGER.value
        )
    ...
    async def test_delivery_confirmation_broadcast(self):
        ...
        confirmation = {
            ...
            "data": {
                ...
                "cylinders_delivered": {"50kg": 2, "20kg": 1},
                "cylinders_collected": {"50kg": 2, "20kg": 1},
            },
        }
    ...
    async def test_batch_updates(self):
        ...
        for i in range(5):
            ws = AsyncMock()
            conn_id = f"office-{i}"
            await websocket_manager.connect(ws, conn_id, i, UserRole.OFFICE_STAFF.value)
        ...
        await websocket_manager.broadcast_to_role(
            json.dumps(batch_update), UserRole.OFFICE_STAFF.value
        )
    ...
    async def test_reconnection_handling(self):
        ...
        await websocket_manager.send_to_user(
            json.dumps({"type": "welcome_back"}), user_id
        )
