...
class TestEInvoiceIntegration:
    ...
    async def test_full_invoice_lifecycle(
        self, mock_government_api, production_invoice
    ):
        ...
        with patch("app.core.einvoice_config.get_einvoice_config") as mock_config:
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                submit_response = mock_government_api(
                    200,
                    {
                        "RtnCode": "1",
                        "RtnMsg": "發票開立成功",
                ...
                query_response = mock_government_api(
                    200,
                    {
                        "RtnCode": "1",
                        "RtnMsg": "查詢成功",
                ...
                # Step 3: Void invoice
                void_response = mock_government_api(
                    200,
                    {
                        "RtnCode": "1",
                        "RtnMsg": "發票作廢成功",
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                with patch("app.core.config.settings") as mock_settings:
                    ...
                    result = await service.issue_allowance(
                        "XY98765432", 1000.0, 50.0, "客戶退貨", items
                    )
        ...
        with patch("app.core.einvoice_config.get_einvoice_config") as mock_config:
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                with patch("app.core.config.settings") as mock_settings:
                    ...
                    assert (
                        len(set(r["einvoice_id"] for r in results)) == 10
                    )  # All unique
