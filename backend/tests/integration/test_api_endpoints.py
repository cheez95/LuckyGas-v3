...
class TestInvoiceAPIIntegration:
    ...
    async def test_invoice_generation_flow(
        ...
        response = await client.post(
            f"/api/v1/invoices/{invoice['id']}/submit",
            headers=auth_headers
        )
        assert response.status_code == 200
        result = response.json()
    ...
    async def test_revenue_report_generation(
        ...
        for i in range(3):
            invoice = {
                ...
                "invoice_date": date.today().isoformat(),
                "status": InvoiceStatus.PAID.value if i < 2 else InvoiceStatus.ISSUED.value
            }
