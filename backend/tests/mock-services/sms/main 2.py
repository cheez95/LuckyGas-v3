...
async def health_check():
    return {"status": "healthy", "service": "mock-sms", "timestamp": datetime.now().isoformat()}
...
async def send_sms(
    ...
    if not (request.to.startswith("09") or request.to.startswith("+8869")):
        raise HTTPException(status_code=400, detail="Invalid Taiwan phone number format")
    
    # Simulate random failures (5% failure rate)
    if random.random() < 0.05:
        raise HTTPException(status_code=500, detail="SMS gateway temporarily unavailable")
