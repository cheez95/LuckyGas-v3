...
async def health_check():
    return {"status": "healthy", "service": "mock-banking", "timestamp": datetime.now().isoformat()}


@app.get("/api/banking/account/{account_number}/balance", response_model=AccountBalanceResponse)
...
async def create_transfer(
    ...
    if random.random() < 0.02:
        raise HTTPException(status_code=500, detail="Banking system temporarily unavailable")
...
async def get_account_transactions(
    ...
    account_txns = [
        ...
        if txn.get("from_account") == account_number or txn.get("to_account") == account_number
    ]
...
async def create_payment_order(request: PaymentOrderRequest):
    ...
    if account["balance"] < request.total_amount:
        raise HTTPException(status_code=400, detail="Insufficient balance for payment order")
...
async def get_payment_order_status(order_id: str):
    ...
    completed_amount = sum(p["amount"] for p in order["payments"] if p["status"] == "completed")
