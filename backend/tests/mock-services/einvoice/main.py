...
async def health_check():
    return {"status": "healthy", "service": "mock-einvoice", "timestamp": datetime.now().isoformat()}
...
def generate_qr_code(invoice_number: str, date: str, random_num: str, amount: float) -> tuple:
...
async def issue_invoice(
    ...
    if abs(calculated_total - request.sales_amount) > 0.01:
        raise HTTPException(status_code=400, detail="Items total does not match sales amount")
