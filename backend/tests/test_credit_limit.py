...
@pytest.mark.asyncio
async def test_order_creation_with_credit_check(async_client: AsyncClient, db: AsyncSession):
...
@pytest.mark.asyncio
async def test_order_creation_with_super_admin_override(async_client: AsyncClient, db: AsyncSession):
...
async def test_get_credit_summary(async_client: AsyncClient, db: AsyncSession):
    ...
    response = await async_client.get(
        f"{settings.API_V1_STR}/orders/credit/{customer.id}",
        headers=headers
    )
