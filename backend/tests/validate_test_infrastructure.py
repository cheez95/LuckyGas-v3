...
class TestInfrastructureValidator:
    ...
    def check_test_database(self) -> bool:
        ...
        try:
            ...
            async def test_connection():
                db_url = os.getenv("DATABASE_URL", "postgresql://test:test@localhost:5432/luckygas_test")
    ...
    def check_test_files(self) -> bool:
        ...
        # Count test files
        test_counts = {
            "unit": len([f for f in os.listdir("tests") if f.startswith("test_") and f.endswith(".py")]),
            "e2e": len([f for f in os.listdir("tests/e2e") if f.startswith("test_") and f.endswith(".py")]),
            "chaos": len([f for f in os.listdir("tests/chaos") if f.startswith("test_") and f.endswith(".py")]),
            "integration": len([f for f in os.listdir("tests/integration") if f.startswith("test_") and f.endswith(".py")])
        }
        ...
        if missing_files:
            ...
            print(f"✅ Test files present - Unit: {test_counts['unit']}, E2E: {test_counts['e2e']}, "
    ...
    def run_sample_tests(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "pytest", "tests/test_main.py", "-v", "-k", "test_health_check"],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def generate_report(self) -> None:
        ...
        with open("TEST_INFRASTRUCTURE_REPORT.md", "w") as f:
            ...
            for check_name, result in self.results["checks"].items():
                ...
                f.write(f"### {check_name.replace('_', ' ').title()} {status_emoji}\n\n")
                f.write(f"- **Status**: {result['status']}\n")
                if "error" in result:
                    f.write(f"- **Error**: {result['error']}\n")
                if "missing_packages" in result:
                    f.write(f"- **Missing Packages**: {', '.join(result['missing_packages'])}\n")
                if "missing_vars" in result:
                    f.write(f"- **Missing Variables**: {', '.join(result['missing_vars'])}\n")
...
def main():
    ...
    print(f"Success Rate: {(validator.passed / (validator.passed + validator.failed) * 100):.1f}%")
    print("\nReports generated:")
