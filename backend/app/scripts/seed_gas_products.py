        ...
        for method in DeliveryMethod:
            for size in [4, 10, 16, 20, 50]:
                for attribute in ProductAttribute:
                    ...
                    # Set Chinese name
                    method_name = "桶裝" if method == DeliveryMethod.CYLINDER else "流量"
                    attr_prefix = {
                    ...
                    method_eng = "Cylinder" if method == DeliveryMethod.CYLINDER else "Flow"
                    product.name_en = f"{size}kg {attr_eng} {method_eng} Gas"
                    
                    # Set description
                    if method == DeliveryMethod.CYLINDER:
                        product.description = f"{product.name_zh} - 桶裝瓦斯，需要瓦斯桶"
                    else:
                        product.description = f"{product.name_zh} - 流量計費，按實際使用量收費"
    ...
    async with async_session_maker() as session:
        result = await session.execute(
            select(GasProduct).order_by(
                GasProduct.delivery_method,
                GasProduct.size_kg,
                GasProduct.attribute
            )
        )
        ...
        for product in products:
            ...
            print(f"{product.sku:<15} {product.name_zh:<30} "
