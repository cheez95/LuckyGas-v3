...
async def seed_driver_test_data():
    ...
    async with async_session() as session:
        ...
        for i, customer in enumerate(customers[:5]):
            ...
            delivery_status = DeliveryStatus.DELIVERED if i < 2 else DeliveryStatus.PENDING
            route_delivery = RouteDelivery(
                ...
                delivered_at=datetime.utcnow() - timedelta(minutes=30) if i < 2 else None,
        ...
        for i, customer in enumerate(customers[5:]):
            ...
            route_delivery = RouteDelivery(
                ...
                estimated_arrival=datetime.utcnow() + timedelta(hours=4, minutes=30 * i)
            )
            session.add(route_delivery)
