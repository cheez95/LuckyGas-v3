...
async def import_customers(session: AsyncSession):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            customer_data = {
                ...
                "invoice_title": (
                    str(row["電子發票抬頭"]) if pd.notna(row["電子發票抬頭"]) else None
                ),
                "short_name": (
                    str(row["客戶簡稱"])
                    if pd.notna(row["客戶簡稱"])
                ...
                # Pricing and payment
                "pricing_method": (
                    str(row["計價方式"]) if pd.notna(row["計價方式"]) else None
                ),
                "payment_method": (
                    str(row["結帳方式"]) if pd.notna(row["結帳方式"]) else None
                ),
                # Status flags
                ...
                # Equipment
                "regulator_model": (
                    str(row["切替器型號"]) if pd.notna(row["切替器型號"]) else None
                ),
    ...
    await session.commit()
    logger.info(
        f"Customer import complete: {imported_count} new, {updated_count} updated"
    )
