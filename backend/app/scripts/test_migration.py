...
def main():
    ...
    try:
        ...
        cylinder_columns = [
            ...
            'cylinders_10kg', 'cylinders_4kg', 'cylinders_ying20',
            'cylinders_ying16', 'cylinders_haoyun20', 'cylinders_haoyun16'
        ]
        ...
        for method, size, attr in required_products:
            product = db.query(GasProduct).filter(
        ...
        customers_with_both = db.query(Customer).join(
            CustomerInventory
        ).filter(
        ...
        if customers_with_both > 0:
            print(f"   ⚠ {customers_with_both} customers have both old cylinder data and inventory records")
