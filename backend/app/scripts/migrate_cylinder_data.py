...
class CylinderDataMigrator:
    ...
    def get_or_create_product(self, specs: Dict) -> Optional[GasProduct]:
        ...
        cache_key = (
            f"{specs['delivery_method']}_{specs['size_kg']}_{specs['attribute']}"
        )
    ...
    def _generate_sku(self, specs: Dict) -> str:
        ...
        method_code = (
            "C" if specs["delivery_method"] == DeliveryMethod.CYLINDER else "F"
        )
        attr_code = {
    ...
    def _generate_name_zh(self, specs: Dict) -> str:
        ...
        method_name = (
            "桶裝" if specs["delivery_method"] == DeliveryMethod.CYLINDER else "流量"
        )
        attr_name = {
    ...
    def migrate_customer(self, customer: Customer) -> Tuple[int, int]:
        ...
        try:
            for column_name, product_specs in self.CYLINDER_MAPPING.items():
                ...
                existing_inventory = (
                    self.db.query(CustomerInventory)
                    .filter(
                ...
                if existing_inventory:
                    ...
                    if not self.dry_run:
                        ...
                        existing_inventory.quantity_owned = (
                            cylinder_count  # Assume all are owned
                        )
                        existing_inventory.quantity_rented = 0
                        existing_inventory.is_active = True

                    logger.debug(
                        f"Updated inventory for customer {customer.customer_code}: "
                    ...
                    logger.debug(
                        f"Created inventory for customer {customer.customer_code}: "
                        f"{product.sku} = {cylinder_count}"
                    )
                    created_count += 1
    ...
    def validate_migration(self) -> bool:
        ...
        customers_with_inventory = (
            self.db.query(Customer).join(CustomerInventory).distinct().count()
        )
        ...
        for item in reversed(self.rollback_data):
            try:
                if item["action"] == "create":
                    ...
                    inventory = (
                        self.db.query(CustomerInventory)
                        .filter(CustomerInventory.id == item["id"])
                        .first()
                    )
                    if inventory:
    ...
    def print_summary(self):
        ...
        logger.info("=" * 50)
        logger.info(
            f"Customers processed: {self.migration_stats['customers_processed']}"
        )
        logger.info(
            f"Inventory records created: {self.migration_stats['inventory_created']}"
        )
        logger.info(
            f"Inventory records updated: {self.migration_stats['inventory_updated']}"
        )
