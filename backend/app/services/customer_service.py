...
class CustomerService:
    ...
    async def create_customer(self, customer_data: CustomerCreate) -> Customer:
        ...
        existing = await self.customer_repo.get_by(customer_code=customer_data.customer_code)
        if existing:
    ...
    async def get_delivery_candidates(
        ...
        for customer in customers:
            ...
            days_since_last = (target_date - last_order.scheduled_date.date()).days if last_order else 999
    ...
    def _calculate_delivery_score(self, customer: Customer, days_since_last: int) -> float:
        """
    ...
    async def identify_vip_customers(self) -> List[Customer]:
        ...
        vip_customers = await self.customer_repo.get_high_value_customers(
            min_monthly_orders=4,
            min_avg_order_value=5000.0
        )
