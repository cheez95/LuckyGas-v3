...
class DataExportService:
    ...
    async def export_orders(
        ...
        if format == "csv":
            ...
            return self._export_multiple_csv(
                {"orders": orders_df, "order_items": items_df}, "orders"
            )
        elif format == "excel":
            return self._export_to_excel(
                {"訂單": orders_df, "訂單明細": items_df}, "orders"
            )
        elif format == "json":
            # Combine data
            combined_data = []
    ...
    async def export_routes(
        ...
        for route in routes:
            route_row = {
                ...
                "開始時間": (
                    format_taiwan_date(route.start_time) if route.start_time else ""
                ),
                "結束時間": (
                    format_taiwan_date(route.end_time) if route.end_time else ""
                ),
            ...
            for stop in route.stops:
                stop_row = {
                    ...
                    "地址": stop.address,
                    "預計到達": (
                        format_taiwan_date(stop.estimated_arrival)
                        if stop.estimated_arrival
                        else ""
                    ),
                    "實際到達": (
                        format_taiwan_date(stop.actual_arrival)
        ...
        if format == "csv":
            return self._export_multiple_csv(
                {"routes": routes_df, "route_stops": stops_df}, "routes"
            )
        elif format == "excel":
            return self._export_to_excel(
                {"路線": routes_df, "路線站點": stops_df}, "routes"
            )
        elif format == "json":
            combined_data = []
            for route in routes:
                    ...
                    "stops": [
                        {
                            ...
                            "customer": (
                                stop.order.customer.name if stop.order else None
                            ),
    ...
    async def export_financial_summary(
        ...
        summary_data = [
            {"項目": "應收帳款總額", "金額": total_invoiced},
            {"項目": "已收款項", "金額": total_paid},
            {"項目": "未收款項", "金額": total_outstanding},
        ]
    ...
    def _export_multiple_csv(
        self, dataframes: Dict[str, pd.DataFrame], filename: str
    ) -> BinaryIO:
    ...
    def _export_to_excel(
        self, sheets: Dict[str, pd.DataFrame], filename: str
    ) -> BinaryIO:
