...
class BankingService:
        ...
        try:
            ...
            self.metrics = {
                "sftp_connections": Gauge(
                    "banking_sftp_connections", "Active SFTP connections", ["bank_code"]
                ),
    ...
    def _get_credentials(self, bank_code: str) -> Dict[str, Any]:
        ...
        if (
            hasattr(self, "_secure_credentials")
            and bank_code in self._secure_credentials
        ):
        ...
        try:
                ...
                if bank_code in self._connection_pool:
                    ...
                    if pool and len(pool) > 0:
                        ...
                        try:
                            sftp.stat(".")
                            if self.metrics:
                                self.metrics["sftp_connections"].labels(
                                    bank_code=bank_code
                                ).inc()
                            yield sftp
                            # Return to pool
            ...
            if self.metrics:
                ...
                self.metrics["sftp_failures"].labels(
                    bank_code=bank_code, error_type=error_type
                ).inc()
    ...
    def _record_circuit_failure(self, bank_code: str):
        ...
        if bank_code not in self._circuit_breaker_states:
            self._circuit_breaker_states[bank_code] = {
                "failures": 0,
                "last_failure": None,
            }
    ...
    def generate_payment_file(self, batch_id: int) -> str:
        ...
        bank_config = (
            self.db.query(BankConfiguration)
            .filter_by(bank_code=batch.bank_code, is_active=True)
            .first()
        )
        ...
        if bank_config.file_format == "fixed_width":
            ...
            raise BankingFormatError(
                f"Unsupported file format: {bank_config.file_format}"
            )
            ...
            writer.writerow(
                {
                    ...
                    "備註": (
                        f"Invoice: {transaction.invoice_id}"
            ...
            for attempt in range(max_retries):
                try:
                    with self.get_sftp_client(bank_config) as sftp:
                        ...
                        logger.info(
                            f"Successfully uploaded payment file {file_name} to {bank_config.bank_code}"
                        )
                        ...
                        if self.metrics:
                            ...
                            self.metrics["transfer_duration"].labels(
                                bank_code=bank_config.bank_code, direction="upload"
                            ).observe(duration)
    ...
    def check_reconciliation_files(self, bank_code: str) -> List[str]:
        ...
        try:
            with self.get_sftp_client(bank_config) as sftp:
                ...
                pattern = self._compile_file_pattern(
                    bank_config.reconciliation_file_pattern
                )
                ...
                logger.info(
                    f"Found {len(new_files)} new reconciliation files for {bank_code}"
                )
    ...
    def process_reconciliation_file(
        self, bank_code: str, file_name: str
    ) -> ReconciliationLog:
        """
        ...
        try:
            ...
            if results:
                ...
                first_transaction = (
                    self.db.query(PaymentTransaction)
                    .filter_by(transaction_id=results[0]["transaction_id"])
                    .first()
                )
            ...
            logger.info(
                f"Processed reconciliation file {file_name}: "
    ...
    def _process_csv_reconciliation(
        self, content: str, bank_config: BankConfiguration
    ) -> List[Dict]:
    ...
    def _generate_file_name(
        self, pattern: str, batch_number: str, date: datetime
    ) -> str:
        """Generate file name from pattern."""
        replacements = {
    ...
    def create_payment_batch(
        ...
        batch_count = (
            self.db.query(func.count(PaymentBatch.id))
            .filter(
        ...
        batch = PaymentBatch(
            ...
            file_format=self.db.query(BankConfiguration.file_format)
            .filter_by(bank_code=bank_code)
            .scalar(),
        )
        ...
        for invoice in invoices:
            transaction = PaymentTransaction(
                ...
                account_holder=invoice.customer.bank_account_holder
        ...
        logger.info(
            f"Created payment batch {batch_number} with {len(invoices)} transactions"
        )
    ...
    def get_payment_status_report(self, batch_id: int) -> Dict[str, Any]:
        ...
        status_summary = {
            status.value: {"count": count, "amount": float(total_amount or 0)}
        ...
        if TransactionStatus.FAILED in status_summary:
            failed = (
                self.db.query(PaymentTransaction)
                .filter_by(batch_id=batch_id, status=TransactionStatus.FAILED)
                .all()
            )
        ...
        return {
            ...
            "reconciled_at": (
                batch.reconciled_at.isoformat() if batch.reconciled_at else None
            ),
