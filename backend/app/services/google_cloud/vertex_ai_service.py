...
class VertexAIDemandPredictionService:
    ...
    async def train_demand_model(self) -> Dict[str, Any]:
        ...
        """
        if not self.gcp_config.is_vertex_ai_configured():
            raise ValueError("Google Cloud not configured. Set GCP_PROJECT_ID and VERTEX_MODEL_ID")
        ...
        async for session in get_async_session():
            ...
            for customer in customers.scalars():
                ...
                for i in range(len(order_list) - 1):
                    ...
                    days_between = (current_order.order_date - next_order.order_date).days
    ...
    async def predict_demand_batch(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async for session in get_async_session():
                ...
                for i, (customer, prediction) in enumerate(zip(customers, predictions.predictions)):
                    ...
                    quantities = await self._calculate_predicted_quantities(customer, session)
            ...
            return {
                ...
                "summary": {
                    "urgent_deliveries": len([p for p in predictions_data if p["is_urgent"]]),
    ...
    async def _prepare_customer_features(self, customer: Customer, session: AsyncSession) -> Dict[str, Any]:
        """
        ...
        avg_daily_usage = customer.avg_daily_usage if hasattr(customer, 'avg_daily_usage') else 1.0
    ...
    async def _calculate_predicted_quantities(self, customer: Customer, session: AsyncSession) -> Dict[str, int]:
        """
        Calculate predicted quantities based on historical patterns
        """
