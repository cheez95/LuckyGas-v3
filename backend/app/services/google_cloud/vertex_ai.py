...
class DemandPredictionService:
    ...
    def __init__(self):
        self.project_id = settings.GCP_PROJECT_ID if hasattr(settings, 'GCP_PROJECT_ID') else "placeholder-project"
        self.location = settings.GCP_LOCATION if hasattr(settings, 'GCP_LOCATION') else "asia-east1"
    ...
    async def predict_demand_batch(self) -> Dict[str, Any]:
        ...
        async for session in get_async_session():
            ...
            for customer in customers:
                ...
                prediction = DeliveryPrediction(
                    ...
                    predicted_date=datetime.combine(prediction_data["predicted_date"], datetime.min.time()),
        ...
        return {
            ...
            "summary": {
                ...
                "average_confidence": sum(p["confidence_score"] for p in predictions_data) / len(predictions_data) if predictions_data else 0
            }
        }
    ...
    async def _generate_customer_prediction(self, customer: Customer) -> Dict[str, Any]:
        ...
        return {
            "customer_id": customer.id,
            "predicted_date": (datetime.utcnow() + timedelta(days=days_until_next)).date(),
