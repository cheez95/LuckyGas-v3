...
class RoutesAPIConfig(BaseModel):
    ...
    compute_matrix_url: str = "https://routes.googleapis.com/distanceMatrix/v2:computeRouteMatrix"
    optimization_url_template: str = "https://routes.googleapis.com/v1/projects/{project}/locations/{location}/routeOptimization:optimizeTours"
    ...
    initial_retry_delay: float = Field(1.0, description="Initial retry delay in seconds")
    ...
    avoid_highways: bool = Field(False, description="Highways are often faster in Taiwan")
    ...
    compute_alternative_routes: bool = Field(False, description="Skip alternatives to save costs")
    requested_reference_routes: List[str] = Field(
        default=["FUEL_EFFICIENT"],
        description="Reference routes for comparison"
    )
    extra_computations: List[str] = Field(
        default=["TOLLS", "FUEL_CONSUMPTION"],
        description="Additional data to compute"
    )
    ...
    # Cost Management
    cost_per_request: float = Field(0.005, description="Estimated cost per API request in USD")
    ...
    business_start_time: time = Field(default=time(8, 0), description="Business start time")
    business_end_time: time = Field(default=time(18, 0), description="Business end time")
    ...
    time_per_cylinder_minutes: int = Field(2, description="Additional time per cylinder")
...
class RouteOptimizationRequest(BaseModel):
    ...
    vehicle_capacity: Optional[Dict[str, int]] = Field(None, description="Vehicle capacity by product type")
    ...
    optimize_for: str = Field("TIME", description="Optimization objective: TIME or DISTANCE")
...
class RouteOptimizationResponse(BaseModel):
    ...
    optimization_savings: Optional[Dict] = Field(None, description="Savings from optimization")
    fuel_consumption_liters: Optional[float] = Field(None, description="Estimated fuel consumption")
