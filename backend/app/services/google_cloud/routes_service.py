...
class GoogleRoutesService:
    ...
    def _build_optimization_request(
        ...
        request = {
            ...
            "requestedReferenceRoutes": ["FUEL_EFFICIENT"]  # Compare with fuel-efficient route
        }
    ...
    def _process_route_response(self, response: Dict, original_stops: List[Dict]) -> Dict:
        ...
        route_info = {
            ...
            "optimized": True,
            "optimization_savings": self._calculate_optimization_savings(route, response)
        }
        
        # Add fuel consumption if available
        if "fuelConsumptionMicroliters" in route:
            route_info["fuel_consumption_liters"] = route["fuelConsumptionMicroliters"] / 1_000_000
    ...
    def _handle_api_error(self, status_code: int, error_text: str):
        ...
        if status_code == 400:
            logger.error(f"Bad Request: {error_data.get('error', {}).get('message', error_text)}")
    ...
    def _calculate_optimization_savings(self, optimized_route: Dict, response: Dict) -> Dict:
        ...
        if len(response.get("routes", [])) > 1:
            ...
            for route in response["routes"][1:]:
                if route.get("routeLabels") and "FUEL_EFFICIENT" in route["routeLabels"]:
                    savings["distance_saved_meters"] = (
                        route.get("distanceMeters", 0) - 
                        optimized_route.get("distanceMeters", 0)
                    )
                    savings["time_saved_minutes"] = (
                        self._parse_duration(route.get("duration", "0s")) - 
                        self._parse_duration(optimized_route.get("duration", "0s"))
                    )
                    if "fuelConsumptionMicroliters" in route and "fuelConsumptionMicroliters" in optimized_route:
    ...
    async def calculate_distance_matrix(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(f"Distance matrix calculation failed (attempt {attempt + 1}): {e}")
                if attempt < self.max_retries - 1:
                    await asyncio.sleep(self.retry_delay * (self.backoff_multiplier ** attempt))
    ...
    def _create_empty_matrix(self, num_origins: int, num_destinations: int) -> Dict[str, Any]:
