...
class RouteOptimizationService:
    ...
    async def optimize_delivery_routes(
        ...
        """
        logger.info(f"Optimizing routes for {len(orders)} orders and {len(vehicles)} vehicles")
        ...
        total_duration = sum(route["estimated_duration_minutes"] for route in route_assignments)
        
        return {
            "optimization_id": f"opt-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}",
            "routes": route_assignments,
            "unassigned_orders": [{"order_id": o.id, "reason": "no_vehicle_available"} for o in unassigned_orders],
            "metrics": {
    ...
    def _group_orders_by_area(self, orders: List[Order]) -> Dict[str, List[Order]]:
        ...
        area_groups = {}
        for order in orders:
            area = order.customer.area if order.customer and order.customer.area else "未分區"
    ...
    async def _create_optimized_route(
        ...
        for idx, order in enumerate(optimized_order):
            ...
            distance = self._calculate_distance(
                current_location,
                self._get_order_location(order)
            )
            ...
            stop = {
                ...
                "address": order.delivery_address or (order.customer.address if order.customer else ""),
                "location": self._get_order_location(order),
                "time_window": {
                    "start": order.delivery_time_start or "09:00",
                    "end": order.delivery_time_end or "17:00"
                },
                "estimated_arrival": (route_date + timedelta(minutes=total_duration + travel_time)).isoformat(),
        ...
        # Add return to depot
        return_distance = self._calculate_distance(current_location, self.depot_location)
        ...
        return {
            ...
            "metrics": {
                ...
                "estimated_end_time": (datetime.combine(route_date.date(), time(8, 0)) + timedelta(minutes=total_duration)).strftime("%H:%M"),
                "vehicle_utilization": min(0.95, len(orders) * 20 / (
                ...
                ) if any([vehicle.max_cylinders_50kg, vehicle.max_cylinders_20kg, 
                         vehicle.max_cylinders_16kg, vehicle.max_cylinders_10kg, 
                         vehicle.max_cylinders_4kg]) else 0.5)  # Mock utilization
            }
        }
    ...
    def _get_order_products(self, order: Order) -> List[Dict[str, Any]]:
        ...
        if hasattr(order, 'order_items') and order.order_items:
            for item in order.order_items:
                products.append({
                    "product_name": item.gas_product.display_name if item.gas_product else f"Product {item.gas_product_id}",
            ...
            if order.qty_50kg > 0:
                products.append({"product_name": "50kg 桶裝瓦斯", "quantity": order.qty_50kg, "size_kg": 50})
            if order.qty_20kg > 0:
                products.append({"product_name": "20kg 桶裝瓦斯", "quantity": order.qty_20kg, "size_kg": 20})
            if order.qty_16kg > 0:
                products.append({"product_name": "16kg 桶裝瓦斯", "quantity": order.qty_16kg, "size_kg": 16})
            if order.qty_10kg > 0:
                products.append({"product_name": "10kg 桶裝瓦斯", "quantity": order.qty_10kg, "size_kg": 10})
            if order.qty_4kg > 0:
                products.append({"product_name": "4kg 桶裝瓦斯", "quantity": order.qty_4kg, "size_kg": 4})
    ...
    async def calculate_eta(
        ...
        return {
            ...
            "eta": (datetime.utcnow() + timedelta(minutes=duration_minutes)).isoformat(),
