...
class MockGoogleRoutesService(GoogleRoutesService):
    ...
    def _nearest_neighbor_with_priority(
        ...
        high_priority.sort(key=lambda s: self._calculate_distance(
        ...
        current_location = (current_location["lat"], current_location["lng"]) if route else depot
        
        while remaining:
            nearest = min(remaining, key=lambda s: self._calculate_distance(
    ...
    async def optimize_multiple_routes(
        ...
        for i, driver in enumerate(drivers):
            ...
            for order in driver_orders:
                if hasattr(order, 'customer') and order.customer:
                    stop = {
                        ...
                        "lat": order.customer.latitude or self.depot_location[0] + random.uniform(-0.1, 0.1),
                        "lng": order.customer.longitude or self.depot_location[1] + random.uniform(-0.1, 0.1),
            ...
            route_data = {
                ...
                "date": date.isoformat(),
                "status": "optimized",
                "area": self._determine_primary_area([s["area"] for s in stops if "area" in s]),
                "stops": optimized["stops"],
                "total_stops": len(optimized["stops"]),
                "total_distance_km": optimized["total_distance"] / 1000,
                "estimated_duration_minutes": int(optimized["total_duration"].rstrip("s")) / 60,
