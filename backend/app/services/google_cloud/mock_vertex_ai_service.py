...
class MockVertexAIDemandPredictionService:
    ...
    async def predict_daily_demand(
        ...
        logger.info(
            f"Mock demand prediction for {len(customer_data)} customers on {prediction_date}"
        )
    ...
    async def predict_customer_churn(
        self, customer_id: int, lookback_days: int = 90
    ) -> Dict[str, Any]:
        """
        ...
        if churn_probability > 0.7:
            ...
            recommendations = ["增加客戶互動頻率", "提供更好的服務", "調查客戶滿意度"]
        else:
            recommendations = ["維持現有服務水準", "定期關懷客戶"]
    ...
    async def optimize_delivery_schedule(
        self, orders: List[Order], constraints: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "metrics": {
                "total_distance_km": sum(
                    r["estimated_distance_km"] for r in optimized_routes
                ),
                "total_duration_minutes": sum(
                    r["estimated_duration_minutes"] for r in optimized_routes
                ),
    ...
    async def analyze_demand_patterns(
        ...
        for date in dates:
            ...
            base_demand = (
                1000
                if not area
                else self.area_base_demand.get(area, {"base": 100})["base"]
            )

            daily_demand = int(
                base_demand * month_factor * dow_factor * random.uniform(0.9, 1.1)
            )
        ...
        patterns = {
            ...
            "seasonal_trend": {
                "direction": (
                    "increasing" if end_date.month > start_date.month else "stable"
                ),
                "strength": "moderate",
            },
        ...
        return {
            ...
            "insights": insights,
            "statistics": {
                "average_daily_demand": sum(d["demand"] for d in trend_data)
                / len(trend_data),
    ...
    async def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "warnings": [
                "This is a mock service for development. Results are simulated."
            ],
        }
