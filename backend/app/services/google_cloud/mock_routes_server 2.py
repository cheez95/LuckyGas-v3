    ...
    async def compute_routes(self, request_data: Dict) -> Dict:
        ...
        if optimize_order and len(intermediates) > 1:
            ...
            sorted_points = sorted(
                intermediate_points,
                key=lambda p: self.calculate_haversine_distance(depot[0], depot[1], p[0], p[1])
            )
            waypoints = [depot] + sorted_points + [waypoints[-1]]
            
            # Generate optimized indices
            optimized_indices = []
            original_intermediates = [(i["location"]["latLng"]["latitude"], i["location"]["latLng"]["longitude"]) 
        ...
        for i in range(len(waypoints) - 1):
            ...
            distance = self.calculate_haversine_distance(start[0], start[1], end[0], end[1])
            ...
            leg = {
                ...
                "polyline": {
                    "encodedPolyline": self.generate_polyline([start, end])
                },
                "startLocation": {
                    "latLng": {
                        "latitude": start[0],
        ...
        route = {
            ...
            "polyline": {
                "encodedPolyline": self.generate_polyline(waypoints)
            }
        }
        ...
        if "FUEL_EFFICIENT" in request_data.get("requestedReferenceRoutes", []):
            fuel_efficient_route = {
                **route,
                ...
                "fuelConsumptionMicroliters": int(fuel_consumption * 0.9 * 1_000_000),  # Less fuel
