...
from app.models import (
    Payment, Invoice, InvoicePaymentStatus, Customer,
    PaymentMethod
)
from app.schemas.payment import (
    PaymentCreate, PaymentUpdate, PaymentStats,
    DailyPaymentSummary, CustomerBalance
)
...
class PaymentService:
    ...
    async def create_payment(
        ...
        await self.db.execute(
            select(Payment).where(Payment.id == payment.id).options(
    ...
    async def update_payment(
        ...
        payment = await self.db.get(Payment, payment_id, options=[
            selectinload(Payment.invoice)
        ])
    ...
    async def get_period_stats(self, period: str) -> PaymentStats:
        ...
        stats = {
            ...
            # By payment method
            "cash_count": sum(1 for p in payments if p.payment_method == PaymentMethod.CASH),
            "cash_amount": sum(p.amount for p in payments if p.payment_method == PaymentMethod.CASH),
            "transfer_count": sum(1 for p in payments if p.payment_method == PaymentMethod.TRANSFER),
            "transfer_amount": sum(p.amount for p in payments if p.payment_method == PaymentMethod.TRANSFER),
            "check_count": sum(1 for p in payments if p.payment_method == PaymentMethod.CHECK),
            "check_amount": sum(p.amount for p in payments if p.payment_method == PaymentMethod.CHECK),
            "credit_card_count": sum(1 for p in payments if p.payment_method == PaymentMethod.CREDIT_CARD),
            "credit_card_amount": sum(p.amount for p in payments if p.payment_method == PaymentMethod.CREDIT_CARD),
            "monthly_count": sum(1 for p in payments if p.payment_method == PaymentMethod.MONTHLY),
            "monthly_amount": sum(p.amount for p in payments if p.payment_method == PaymentMethod.MONTHLY),
            ...
            "daily_average_amount": sum(p.amount for p in payments) / ((end_date - start_date).days + 1)
        }
