...
class DataImportService:
    ...
    async def import_customers(
        ...
        optional_columns = [
            "地址", "區域", "客戶類型", "信用額度", "付款條件",
            "聯絡人", "聯絡人電話", "電子郵件", "統一編號", "備註"
        ]
        ...
        for index, row in df.iterrows():
            try:
                ...
                customer_data = {
                    ...
                    "customer_type": self._map_customer_type(row.get("客戶類型", "individual")),
                    ...
                    "contact_phone": self._clean_phone(str(row.get("聯絡人電話", ""))) or None,
    ...
    async def import_products(
        ...
        optional_columns = ["類別", "規格", "單位", "成本", "庫存量", "最低庫存", "描述"]
    ...
    async def import_orders(
        ...
        if file_type == "csv":
            ...
            raise ValidationError("Order import requires Excel file with two sheets: Orders and Order Items")
        elif file_type in ["excel", "xlsx", "xls"]:
            # Read both sheets
            try:
                ...
                raise ValidationError(f"Excel file must contain '訂單' and '訂單明細' sheets: {str(e)}")
        else:
            raise ValueError(f"Unsupported file type: {file_type}")
        ...
        if missing_columns:
            raise ValidationError(f"Missing required columns in orders sheet: {missing_columns}")
        ...
        if missing_columns:
            raise ValidationError(f"Missing required columns in items sheet: {missing_columns}")
        ...
        for index, row in items_df.iterrows():
            try:
                ...
                item_data = {
                    ...
                    "quantity": quantity,
                    "unit_price": self._parse_decimal(row.get("單價", product.unit_price)),
    ...
    def _read_csv(self, file: BinaryIO) -> pd.DataFrame:
        ...
        raise ValidationError("Unable to decode CSV file. Please ensure it's saved in UTF-8 encoding.")
