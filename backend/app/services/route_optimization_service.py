...
class RouteOptimizationService:
    ...
    async def optimize_routes(
        ...
        try:
            ...
            distance_matrix = await self._create_distance_matrix(orders, constraints["depot_location"])
            
            # Create time matrix (considering traffic)
            time_matrix = await self._create_time_matrix(orders, constraints["depot_location"])
            
            # Create demand array (cylinder quantities)
            demands = [0] + [order.get("quantity", 1) for order in orders]  # 0 for depot
            ...
            # Format solution
            routes = self._format_solution(solution, orders, drivers, distance_matrix, time_matrix)
        ...
        for vehicle_id in range(num_vehicles):
            ...
            if route:
                routes.append({
                    ...
                    "route": route,
                    "distance": solution.Value(routing.GetDimensionOrDie('Distance').CumulVar(index)),
        ...
        for route_data in solution.get("routes", []):
            ...
            for i, order_idx in enumerate(route_data["route"]):
                ...
                stops.append({
                    ...
                    "distance_from_prev": distance_matrix[prev_idx][curr_idx] / 1000,  # km
                })
            ...
            formatted_routes.append({
                ...
                "total_time": current_time - 480 + time_matrix[prev_idx][0],  # minutes
    ...
    def _calculate_metrics(
        ...
        optimization_score = min(
            (min_spanning_tree_distance / total_distance * 100) if total_distance > 0 else 0,
            100
        )
