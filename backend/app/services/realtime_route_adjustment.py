...
class RealtimeRouteAdjustmentService:
                ...
                if optimized and optimized.total_duration_minutes < new_time:
                    ...
                    return AdjustmentResult(
                        ...
                        changes=[{
                            ...
                            "reason": "traffic",
                            "time_saved": new_time - optimized.total_duration_minutes
                        }],
                        new_total_distance=route.total_distance_km,
                        new_total_time=route.total_duration_minutes,
    ...
    async def _find_best_route_for_order(
        ...
        stmt = select(Route).where(
            ...
            Route.status.in_(["not_started", "in_progress"])
        )
    ...
    async def _recalculate_route(self, route: Route, session: AsyncSession):
        ...
        for i, stop in enumerate(route.stops):
            ...
            current_time = stop.estimated_arrival + timedelta(minutes=stop.estimated_duration)
    ...
    async def _get_route(self, route_id: str, session: Optional[AsyncSession] = None) -> Optional[Route]:
        """Get route by ID."""
    ...
    async def _calculate_distance(self, from_loc: Tuple[float, float], to_loc: Tuple[float, float]) -> float:
    ...
    async def _handle_driver_unavailable(self, request: AdjustmentRequest) -> AdjustmentResult:
        """Handle driver unavailability by reassigning routes."""
        # Implementation for driver unavailability
        pass
        
    async def _handle_generic_adjustment(self, request: AdjustmentRequest) -> AdjustmentResult:
