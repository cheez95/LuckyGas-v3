...
class RouteAnalyticsService:
    ...
    async def get_daily_analytics(
        self, target_date: date, session: AsyncSession
    ...
    async def get_weekly_trends(
        self, end_date: date, session: AsyncSession
    ) -> WeeklyTrendReport:
        ...
        on_time_pcts = [
            s.on_time_percentage for s in daily_summaries if s.total_routes > 0
        ]
    ...
    async def get_driver_performance(
        ...
        total_completed = sum(
            1 for r in routes for s in r.stops if s.status == "completed"
        )

        on_time_pct = (
            (on_time_deliveries / total_completed * 100) if total_completed > 0 else 0
        )
        ...
        return DriverPerformanceMetrics(
            ...
            daily_breakdown=await self._get_driver_daily_breakdown(
                driver_id, start_date, end_date, session
            ),
        )

    async def get_fuel_savings_report(
        self, start_date: date, end_date: date, session: AsyncSession
    ) -> FuelSavingsReport:
        ...
        return FuelSavingsReport(
            ...
            savings_percentage=(
                (distance_saved / total_baseline_distance * 100)
                if total_baseline_distance > 0
                else 0
            ),
            daily_breakdown=await self._get_daily_fuel_breakdown(
                start_date, end_date, session
            ),
            vehicle_type_breakdown=await self._get_vehicle_fuel_breakdown(
                start_date, end_date, session
            ),
        )

    async def _calculate_fuel_savings(
        self, routes: List[Route], session: AsyncSession
    ) -> Dict[str, float]:
    ...
    async def _get_optimization_metrics(
        ...
        savings_percentages = [
            opt.savings_percentage for opt in optimizations if opt.savings_percentage
        ]
        ...
        for route in routes:
            ...
            for stop in route.stops:
                if stop.status == "completed":
                    ...
                    if (
                        stop.actual_arrival
                        and stop.actual_arrival <= stop.estimated_arrival
                    ):
                        stats["on_time"] += 1
    ...
    def _generate_suggestions(
        self, summaries: List[DailyAnalyticsSummary]
    ) -> List[str]:
        ...
        if avg_on_time < 80:
            suggestions.append(
                "Consider adjusting time windows or adding buffer time to routes"
            )
        ...
        if total_fuel_saved < 100:
            suggestions.append(
                "Increase route optimization frequency to improve fuel savings"
            )
        ...
        if max(daily_deliveries) > 2 * min(daily_deliveries):
            suggestions.append(
                "Consider load balancing across weekdays to improve efficiency"
            )
    ...
    async def _get_driver_daily_breakdown(
        self, driver_id: str, start_date: date, end_date: date, session: AsyncSession
    ) -> List[Dict]:
        ...
        while current <= end_date:
            stmt = (
                select(Route)
                .options(selectinload(Route.stops))
                .where(and_(Route.driver_id == driver_id, Route.route_date == current))
            )
    ...
    async def _get_vehicle_fuel_breakdown(
        ...
        # Mock implementation
        return {
            "small": {"total_distance": 1200, "fuel_saved": 50, "cost_saved": 1500},
            "medium": {"total_distance": 2400, "fuel_saved": 120, "cost_saved": 3600},
            "large": {"total_distance": 800, "fuel_saved": 60, "cost_saved": 1800},
        }
