...
from app.models import (
    Order, Customer, User, Route, DeliveryHistory, 
    Payment, Invoice, GasProduct, Vehicle, OrderItem,
    CustomerInventory
)
...
class AnalyticsService:
    ...
    async def get_revenue_metrics(
        ...
        product_query = select(
            ...
            GasProduct, OrderItem.product_id == GasProduct.id
        ).where(
            and_(
                Order.created_at >= start_date,
                Order.created_at <= end_date,
        ...
        return {
            ...
            "averageOrderValue": float(current_revenue / revenue_data.order_count) if revenue_data.order_count else 0,
    ...
    async def get_order_metrics(
        ...
        return {
            ...
            "completionRate": round(
                (status_breakdown.get(OrderStatus.COMPLETED, 0) / total_orders * 100) 
    ...
    async def get_customer_metrics(
        ...
        top_customers_query = select(
            ...
            Customer.id, Customer.name, Customer.business_name
        ...
        customer_segments = {
            row.customer_type: row.count for row in segment_result
        }
    ...
    async def get_cash_flow_metrics(
        ...
        daily_inflows = {
            row.date.isoformat(): float(row.amount or 0) 
        ...
        return {
            ...
            "collectionRate": round(
                ...
                if (total_inflow + outstanding_receivables) > 0 else 0,
                2
            )
        }
        ...
        return {
            ...
            "monthOverMonth": {
                ...
                "customers": calculate_change(
                    ...
                    prev_metrics["activeCustomers"]
                )
            },
            "yearOverYear": {
                "revenue": calculate_change(
                    current_metrics["revenue"], 
                    year_ago_metrics["revenue"]
                ),
                "orders": calculate_change(
                    current_metrics["orders"], 
                    year_ago_metrics["orders"]
                ),
                "customers": calculate_change(
                    current_metrics["activeCustomers"], 
    ...
    async def get_top_performing_metrics(
        ...
        routes_query = select(
            ...
            DeliveryHistory, Route.id == DeliveryHistory.route_id
        ).where(
            and_(
        ...
        ).group_by(Route.id, Route.name).order_by(
            func.count(DeliveryHistory.id).desc()
        ).limit(5)
        ...
        drivers_query = select(
            ...
            DeliveryHistory, User.id == DeliveryHistory.driver_id
        ).where(
            and_(
            ...
            func.count(DeliveryHistory.id) > 0
        ).order_by(
            (func.sum(
                case(
                    (DeliveryHistory.status == DeliveryStatus.COMPLETED, 1),
        ...
        top_drivers = [
            {
                ...
                "completionRate": round(
                    ...
                    if row.delivery_count > 0 else 0, 
                    2
                )
        ...
        ).group_by(GasProduct.id, GasProduct.name).order_by(
    ...
    async def get_realtime_order_status(self, date: datetime) -> Dict[str, Any]:
        ...
        recent_query = select(Order).options(
    ...
    async def get_driver_utilization(self, date: datetime) -> Dict[str, Any]:
        ...
        active_drivers_query = select(
            func.count(func.distinct(Route.driver_id))
        ).where(
            and_(
                Route.date == date.date(),
        ...
        for row in driver_result:
            ...
            elif row.status == RouteStatus.COMPLETED and driver_summary[row.id]["status"] == "idle":
    ...
    async def get_route_efficiency_metrics(self, date: datetime) -> Dict[str, Any]:
        ...
        routes_query = select(
            ...
            Route.id, Route.name, Route.status,
            Route.planned_start_time, Route.actual_start_time,
            Route.planned_end_time, Route.actual_end_time
        )
        ...
        for row in routes_result:
            ...
            if row.actual_start_time and row.planned_start_time:
                start_delay = (row.actual_start_time - row.planned_start_time).total_seconds() / 60
                if start_delay > 15:  # 15 minutes tolerance
                    on_time = False
            
            if on_time and row.actual_end_time and row.planned_end_time:
                end_delay = (row.actual_end_time - row.planned_end_time).total_seconds() / 60
        ...
        on_time_rate = round(
            (on_time_routes / total_routes * 100) 
    ...
    async def get_delivery_success_metrics(self, date: datetime) -> Dict[str, Any]:
        ...
        status_query = select(
            ...
            func.count(DeliveryHistory.id).label('count')
        ).where(
            and_(
        ...
        ).group_by(DeliveryHistory.notes).limit(5)
        
        failure_result = await self.db.execute(failure_query)
        failure_reasons = [
            {
                "reason": row.notes or "未指定原因",
                "count": row.count
    ...
    async def get_equipment_inventory_status(self) -> Dict[str, Any]:
        ...
        ).select_from(GasProduct).outerjoin(
            CustomerInventory, GasProduct.id == CustomerInventory.product_id
        ...
        for row in inventory_result:
            ...
            inventory_status.append({
                ...
                "utilizationRate": round(
                    (customer_inv / total * 100) if total > 0 else 0,
                    2
                )
            })
    ...
    async def get_operational_alerts(self) -> List[Dict[str, Any]]:
        ...
        low_inventory_query = select(
            GasProduct.name,
            func.sum(CustomerInventory.quantity).label('total')
        ).select_from(GasProduct).outerjoin(
            CustomerInventory, GasProduct.id == CustomerInventory.product_id
        ).group_by(GasProduct.id, GasProduct.name).having(
            func.sum(CustomerInventory.quantity) < 50  # Threshold
        )
        ...
        failed_deliveries_query = select(
    ...
    async def get_accounts_receivable_aging(self) -> Dict[str, Any]:
        ...
        debtors_query = select(
            ...
            func.sum(Invoice.total_amount - Invoice.paid_amount).desc()
        ).limit(10)
        ...
        payment_methods = [
            {
                ...
                "count": row.count,
                "percentage": round(
                    (row.amount / total_collected * 100) 
    ...
    async def get_outstanding_invoices(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "current": len([inv for inv in outstanding_invoices if not inv["isOverdue"]]),
    ...
    async def get_revenue_by_segment(
        ...
        for row in segment_result:
            ...
            segments.append({
                ...
                "revenue": float(revenue),
                "orderCount": row.order_count,
                "averageOrderValue": float(revenue / row.order_count) if row.order_count > 0 else 0
            })
        
        # Add percentage to each segment
        for segment in segments:
            segment["percentage"] = round(
                (segment["revenue"] / total_revenue * 100) 
        ...
        ).group_by(Customer.area).order_by(
    ...
    async def get_api_usage_statistics(
        ...
        endpoints = [
            {"endpoint": "/api/v1/orders", "calls": 15234, "avgTime": 125, "errors": 23},
            {"endpoint": "/api/v1/customers", "calls": 8932, "avgTime": 98, "errors": 12},
            {"endpoint": "/api/v1/routes", "calls": 6543, "avgTime": 234, "errors": 8},
            {"endpoint": "/api/v1/deliveries", "calls": 12876, "avgTime": 156, "errors": 34},
            {"endpoint": "/api/v1/analytics", "calls": 2341, "avgTime": 456, "errors": 5}
        ]
        ...
        return {
            ...
            "errorRate": round(total_errors / total_calls * 100, 2) if total_calls > 0 else 0,
            "endpoints": endpoints,
            "peakHours": [
                {"hour": 9, "calls": 2341},
                {"hour": 10, "calls": 3456},
    ...
    async def get_custom_metrics(
        ...
        for metric in metrics:
            if metric == "revenue":
                results["revenue"] = await self.get_revenue_metrics(start_date, end_date)
            elif metric == "orders":
                results["orders"] = await self.get_order_metrics(start_date, end_date)
            elif metric == "customers":
                results["customers"] = await self.get_customer_metrics(start_date, end_date)
    ...
    async def generate_report(
        ...
        if report_type == "executive":
            data = {
                ...
                "performance": await self.get_performance_comparison(start_date, end_date)
            }
        elif report_type == "financial":
            data = {
                "receivables": await self.get_accounts_receivable_aging(),
                "collections": await self.get_payment_collection_metrics(start_date, end_date),
    ...
    async def _generate_pdf_report(
        ...
        # This is a placeholder
        return f"/tmp/{report_type}_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
