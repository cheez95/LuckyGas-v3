...
from app.models import (
    ...
    InvoiceStatus, InvoicePaymentStatus, Customer, Order
)
...
class InvoiceService:
    ...
    async def create_invoice(
        ...
        invoice = Invoice(
            **invoice_numbers,
            ...
            due_date=invoice_data.due_date or invoice_data.invoice_date + timedelta(days=30),
    ...
    async def issue_invoice(
        self,
        invoice_id: int,
        einvoice_service: Any
    ) -> Invoice:
        """Issue an invoice to government e-invoice platform"""
        invoice = await self.db.get(Invoice, invoice_id, options=[
            selectinload(Invoice.items),
            selectinload(Invoice.customer)
        ])
    ...
    async def get_period_stats(self, period: str) -> InvoiceStats:
        ...
        stats = {
            ...
            "paid_count": sum(1 for inv in invoices if inv.payment_status == InvoicePaymentStatus.PAID),
            "paid_amount": sum(inv.paid_amount for inv in invoices),
            "unpaid_count": sum(1 for inv in invoices if inv.payment_status in [InvoicePaymentStatus.PENDING, InvoicePaymentStatus.PARTIAL]),
            "unpaid_amount": sum(inv.total_amount - inv.paid_amount for inv in invoices),
            "overdue_count": sum(1 for inv in invoices if inv.payment_status == InvoicePaymentStatus.OVERDUE),
            "overdue_amount": sum(inv.total_amount - inv.paid_amount for inv in invoices if inv.payment_status == InvoicePaymentStatus.OVERDUE),
            "b2b_count": sum(1 for inv in invoices if inv.invoice_type == "B2B"),
            "b2b_amount": sum(inv.total_amount for inv in invoices if inv.invoice_type == "B2B"),
            "b2c_count": sum(1 for inv in invoices if inv.invoice_type == "B2C"),
            "b2c_amount": sum(inv.total_amount for inv in invoices if inv.invoice_type == "B2C"),
        }
