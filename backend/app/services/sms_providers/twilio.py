...
class TwilioProvider:
        ...
        try:
            ...
            async with self._session.post(url, data=urlencode(data)) as response:
                ...
                if response.status == 201:
                    ...
                    cost_usd = float(result.get("price", 0)) * -1  # Twilio returns negative
        ...
        try:
            ...
            async with self._session.get(url) as response:
                if response.status == 200:
                    ...
                    our_status = status_map.get(twilio_status, NotificationStatus.PENDING)
    ...
    async def register_delivery_callback(self, message_id: str, callback_url: str) -> bool:
        ...
        logger.info(f"Twilio uses StatusCallback during send, cannot update for {message_id}")
        return False
