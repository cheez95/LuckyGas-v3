    ...
    async def check_status(self, message_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            xml_data = self._build_status_xml(
                msg_id=message_id, timestamp=timestamp, signature=signature
            )
                ...
                if response.status == 200:
                    ...
                    if result["success"]:
                        ...
                        our_status = status_map.get(
                            cht_status, NotificationStatus.PENDING
                        )
...
<BalanceQuery>
    ...
    def _parse_balance_response(self, xml_response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            status = root.find("Status")
            if status is not None and status.text == "0":
                return {"success": True, "balance": int(root.find("Balance").text or 0)}
            else:
