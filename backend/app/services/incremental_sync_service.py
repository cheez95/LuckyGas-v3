...
class IncrementalSyncService:
    ...
    def __init__(
        ...
        self.sync_config = {
            'clients': {
                ...
                'text_fields': ['client_code', 'invoice_title', 'short_name', 'address',
        ...
        for start_idx in range(0, len(legacy_changes), self.batch_size):
            ...
            for _, record in batch.iterrows():
                try:
                    ...
                    logger.error(f"Failed to sync {table_name} record {record.get('id')}: {e}")
                    self.stats['errors'] += 1
    ...
    async def sync_new_to_legacy(
        ...
        try:
            ...
            await self.set_last_sync_time(
                f"{config['timestamp_key']}:reverse",
                max_timestamp
            )
    ...
    async def sync_customer_record(
        ...
        if new_id:
            ...
            if customer:
                ...
                if hasattr(customer, 'updated_at') and customer.updated_at:
                    record_updated = pd.to_datetime(record.get('updated_at', datetime.min))
                    if customer.updated_at > record_updated:
                        logger.debug(f"Skipping customer {record['client_code']} - newer version exists")
        ...
        customer = Customer(
            ...
            monthly_delivery_volume=float(record.get('monthly_delivery_volume', 0)) if pd.notna(record.get('monthly_delivery_volume')) else None,
            daily_usage_avg=float(record.get('daily_usage_avg', 0)) if pd.notna(record.get('daily_usage_avg')) else None,
    ...
    async def sync_driver_record(
        ...
        if new_id:
            ...
            if user:
                ...
                user.driver_experience_years = int(record.get('experience_years', 0)) if pd.notna(record.get('experience_years')) else None
        ...
        user = User(
            ...
            driver_experience_years=int(record.get('experience_years', 0)) if pd.notna(record.get('experience_years')) else None,
            driver_familiar_areas=self._parse_familiar_areas(record.get('familiar_areas')),
    ...
    async def sync_vehicle_record(
        ...
        if pd.notna(record.get('driver_id')):
            driver_id = await self.get_id_mapping(
                self.sync_config['drivers']['id_mapping_key'],
        ...
        vehicle = Vehicle(
            ...
            last_maintenance_date=pd.to_datetime(record.get('last_maintenance')) if pd.notna(record.get('last_maintenance')) else None,
            next_maintenance_date=pd.to_datetime(record.get('next_maintenance')) if pd.notna(record.get('next_maintenance')) else None,
            created_at=pd.to_datetime(record.get('created_at', datetime.now())),
            updated_at=pd.to_datetime(record.get('updated_at', datetime.now()))
        )
