...
class SMSGateway:
    ...
    async def initialize(self):
        ...
        self._primary_provider = SMSProvider(settings.SMS_PRIMARY_PROVIDER) if settings.SMS_PRIMARY_PROVIDER else None
        self._fallback_providers = [SMSProvider(p) for p in settings.SMS_FALLBACK_PROVIDERS if p]
        
        logger.info(f"SMS Gateway initialized with providers: {list(self._providers.keys())}")
    ...
    async def send_sms(
        ...
        for provider in self._fallback_providers:
            if provider in self._providers and await self._check_rate_limit(provider, priority):
                result = await self._send_with_provider(
                    provider,
                    phone,
                    message,
    ...
    async def check_delivery_status(self, message_id: str, provider: SMSProvider) -> Dict[str, Any]:
