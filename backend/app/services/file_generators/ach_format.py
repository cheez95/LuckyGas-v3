...
class TaiwanACHGenerator:
    ...
    def __init__(self, company_code: str = "LUCKYGAS", company_name: str = "幸福氣體有限公司"):
        self.company_code = company_code
        self.company_name = company_name
    
    def generate_payment_file(self, batch: PaymentBatch, encoding: str = 'big5') -> bytes:
        """
        ...
        # Generate trailer
        trailer = self._create_trailer(len(batch.transactions), total_amount, hash_total)
    ...
    def _create_detail(self, transaction: PaymentTransaction, sequence: int) -> ACHDetail:
        ...
        if transaction.customer:
            id_number = transaction.customer.tax_id or transaction.customer.national_id or ''
        ...
        return ACHDetail(
            ...
            id_number=id_number,
            payment_description=f'Gas Bill {transaction.invoice_id}' if transaction.invoice_id else 'Gas Payment',
            customer_reference=str(transaction.customer_id) if transaction.customer_id else ''
        )
    
    def _create_trailer(self, record_count: int, total_amount: Decimal, hash_total: int) -> ACHTrailer:
    ...
    def _parse_account_codes(self, account_number: str, bank_code: Optional[str]) -> tuple:
        """
    ...
    def validate_batch(self, batch: PaymentBatch) -> List[str]:
        ...
        for idx, transaction in enumerate(batch.transactions):
            ...
            if not transaction.account_holder:
                ...
                logger.warning(f"Transaction {idx+1}: Account holder name will be truncated")
            
            # Validate amount
            if transaction.amount <= 0:
                errors.append(f"Transaction {idx+1}: Invalid amount {transaction.amount}")
...
class ACHReconciliationParser:
    ...
    def parse_reconciliation_file(self, content: bytes, encoding: str = 'big5') -> List[Dict]:
        """
