...
class OrderService:
    ...
    async def update_delivery_status(
        ...
        delivered_at = datetime.utcnow() if order_status == OrderStatus.DELIVERED else None
    ...
    async def get_pending_payments(
        ...
        """
        orders = await self.order_repo.get_pending_payments(
            customer_id, days_overdue
        )
        
        pending_payments = []
        for order in orders:
            days_pending = (datetime.now() - order.delivered_at).days if order.delivered_at else 0
            
            pending_payments.append({
                "order": order,
                "customer_name": order.customer.short_name if order.customer else "Unknown",
