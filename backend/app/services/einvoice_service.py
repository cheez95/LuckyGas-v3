...
class EInvoiceService:
    ...
    async def _mock_submit_invoice(self, invoice: Invoice) -> Dict[str, Any]:
        ...
        logger.info(f"Mock mode: Simulating invoice submission for {invoice.invoice_number}")
        
        # Simulate some validation
        if not invoice.invoice_number:
            raise ValueError("發票號碼不能為空")
        
        if not self.validate_tax_id(invoice.buyer_tax_id) and invoice.invoice_type == InvoiceType.B2B:
