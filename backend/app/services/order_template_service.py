...
class OrderTemplateService:
    ...
    async def create_template(
        ...
        if not template_data.template_code:
            template_count = await db.execute(
                select(func.count()).select_from(OrderTemplate)
                .where(OrderTemplate.customer_id == template_data.customer_id)
            )
            count = template_count.scalar() or 0
            template_data.template_code = f"{customer.customer_code}_TPL_{count + 1:03d}"
        ...
        if template.is_recurring:
            template.next_scheduled_date = OrderTemplateService._calculate_next_scheduled_date(
        ...
        logger.info(f"Created order template {template.template_code} for customer {customer.customer_code}")
        return template
    ...
    async def get_template(
        ...
        query = select(OrderTemplate).options(
    ...
    async def list_templates(
        ...
        # Apply pagination
        query = query.order_by(OrderTemplate.times_used.desc(), OrderTemplate.created_at.desc())
    ...
    async def create_order_from_template(
        ...
        order_data = OrderCreateV2(
            ...
            is_urgent=request.override_priority == "urgent" or template.priority == "urgent",
        ...
        logger.info(f"Created order {order.order_number} from template {template.template_code}")
        return order
