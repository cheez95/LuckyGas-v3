...
class ORToolsOptimizer:
    ...
    def optimize(
        ...
        max_vehicle_time = (
            max(data["vehicle_max_time"]) if data["vehicle_max_time"] else 3600
        )
        ...
        for location_idx, time_window in enumerate(data["time_windows"]):
            ...
            if end_time < 0:
                logger.warning(
                    f"Negative end time ({end_time}), setting to max_vehicle_time"
                )
            ...
            if end_time > max_vehicle_time:
                logger.warning(
                    f"End time ({end_time}) exceeds max_vehicle_time ({max_vehicle_time}), capping to max"
                )
                end_time = int(max_vehicle_time)

            if start_time > max_vehicle_time:
                logger.warning(
                    f"Start time ({start_time}) exceeds max_vehicle_time ({max_vehicle_time}), capping to max"
                )
        ...
        # Solve
        logger.info(
            f"Optimizing routes for {len(stops)} stops and {len(vehicles)} vehicles"
        )
    ...
    def _calculate_route_load(self, stops: List[VRPStop]) -> str:
        ...
        return ", ".join(
            [f"{qty} x {product}" for product, qty in total_demand.items() if qty > 0]
        )
...
ortools_optimizer = ORToolsOptimizer(depot_location=(25.0330, 121.5654))  # Taipei
