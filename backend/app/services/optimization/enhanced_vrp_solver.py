...
class OptimizationConfig:
    ...
    avg_speed_kmh: Dict[str, float] = field(default_factory=lambda: {
        "peak": 20,
        "normal": 35,
        "highway": 60
    })
    ...
    # Vehicle constraints
    default_vehicle_capacity: Dict[str, int] = field(default_factory=lambda: {
        "50kg": 20,
        "20kg": 50,
        "16kg": 60,
        "10kg": 100,
    ...
    service_time_per_cylinder: Dict[str, int] = field(default_factory=lambda: {
        "50kg": 5,
        "20kg": 3,
        "16kg": 3,
        "10kg": 2,
...
class EnhancedVRPSolver:
    ...
    async def optimize_routes(
        ...
        try:
            ...
            optimization_time_ms = int((datetime.now() - start_time).total_seconds() * 1000)
    ...
    def _balance_driver_workload(
        ...
        for vehicle_id, stops in assignments.items():
            total_demand = sum(
                sum(stop.demand.values()) for stop in stops
            )
        ...
        overloaded = [(v, w) for v, w in workloads.items() 
                      if w['score'] > avg_score * 1.2]
        underloaded = [(v, w) for v, w in workloads.items() 
                       if w['score'] < avg_score * 0.8]
    ...
    async def _get_detailed_routes(
        ...
        for vehicle_id, stops in assignments.items():
            ...
            try:
                ...
                logger.error(f"Failed to get Google route for vehicle {vehicle_id}: {e}")
    ...
    def _calculate_optimization_metrics(
        ...
        distance_efficiency = min(1.0, 5.0 / avg_distance_per_stop) if avg_distance_per_stop else 0
        time_efficiency = min(1.0, 480 / (total_duration / len(routes))) if routes else 0
        ...
        return {
            ...
            'avg_duration_per_route': round(total_duration / len(routes), 1) if routes else 0,
    ...
    def _parse_time_window(
        ...
        if start_time:
            try:
                ...
                start_minutes = hours * 60 + minutes - 480  # Subtract 8 hours (8 AM start)
        ...
        if end_time:
            try:
                ...
                end_minutes = max(start_minutes + 30, end_minutes)  # At least 30 min window
            except:
        ...
        for i, driver in enumerate(drivers):
            ...
            for j, order in enumerate(driver_orders):
                ...
                stop = RouteStop(
                    ...
                    estimated_arrival=optimization_date.replace(hour=8) + timedelta(minutes=j*30),
