...
from app.services.optimization.ortools_optimizer import ORToolsOptimizer, VRPStop, VRPVehicle
...
class VRPOptimizer:
    ...
    async def optimize_routes(
        ...
        try:
            ...
            if not orders:
                return self._create_empty_response(optimization_id, "No valid orders found")
            
            if not vehicles:
                return self._create_empty_response(optimization_id, "No available vehicles")
            ...
            if len(orders) > 20:  # Cluster for larger order sets
                ...
                logger.info(f"Created {len(clusters)} clusters for {len(orders)} orders")
                await self._send_progress(optimization_id, 40, f"Created {len(clusters)} clusters")
            ...
            # Run optimization
            await self._send_progress(optimization_id, 50, "Running route optimization...")
    ...
    async def _cluster_orders(
        ...
        if request.respect_time_windows:
            return self.clusterer.cluster_by_time_windows(
                locations,
                eps_km=request.cluster_radius_km
            )
        else:
    ...
    def _convert_to_vrp_stops(
        ...
        for order in orders:
            ...
            time_window = self._get_time_window_minutes(
                order.get("delivery_time_slot", "anytime"),
                constraints
            )
    ...
    def _get_time_window_minutes(
        ...
        shift_start_minutes = constraints.driver_shift_start.hour * 60 + constraints.driver_shift_start.minute
        shift_end_minutes = constraints.driver_shift_end.hour * 60 + constraints.driver_shift_end.minute
        
        if time_slot == "morning":
            return (0, 180)  # First 3 hours
        elif time_slot == "afternoon":
            lunch_end = constraints.lunch_break_start.hour * 60 + constraints.lunch_break_start.minute + constraints.lunch_break_duration_minutes
            return (lunch_end - shift_start_minutes, shift_end_minutes - shift_start_minutes)
    ...
    def _apply_taiwan_optimizations(
        ...
        if weekday in [self.taiwan_settings["market_days"]["wednesday"],
            ...
            for stop in stops:
                if self._is_near_school(stop.latitude, stop.longitude):
                    ...
                    if stop.time_window[0] < 120:  # Morning window
                        stop.time_window = (120, stop.time_window[1])  # Start after 9 AM
    ...
    async def _optimize_clustered_routes(
        ...
        for cluster, assigned_vehicles in cluster_assignments.items():
            cluster_stops = [stop_map[oid] for oid in cluster.order_ids if oid in stop_map]
            if cluster_stops and assigned_vehicles:
                task = self._optimize_cluster_with_timeout(
                    cluster_stops,
                    assigned_vehicles,
                    request.optimization_mode,
                    timeout_seconds=min(30, 5 + len(cluster_stops) * 0.1)  # Dynamic timeout
                )
    ...
    def _assign_vehicles_to_clusters(
        ...
        sorted_clusters = sorted(
            clusters,
            key=lambda c: sum(c.total_demand.values()),
            reverse=True
        )
            ...
            for _ in range(min(vehicles_needed, len(available_vehicles))):
                ...
                closest_vehicle = min(
                    ...
                    key=lambda v: self._calculate_distance(
                        v.start_location[0], v.start_location[1],
    ...
    async def _optimize_cluster_with_timeout(
        ...
        try:
            ...
            logger.warning(f"Cluster optimization timed out after {timeout_seconds}s, using fallback")
    ...
    async def _optimize_cluster(
        ...
        routes = await loop.run_in_executor(
            None,
            self.ortools_optimizer.optimize,
            stops,
            vehicles
        )
            ...
            for task in done:
                try:
                    ...
                    if done_count >= total_tasks * 0.8:  # 80% complete
                        logger.info(f"Early termination: {done_count}/{total_tasks} tasks complete")
    ...
    async def _build_optimization_response(
        ...
        for vehicle_idx, stops in optimized_routes.items():
            ...
            for seq, stop in enumerate(stops):
                ...
                if seq > 0:
                    travel_time = await self._estimate_travel_time(
                        stops[seq-1].latitude, stops[seq-1].longitude,
            ...
            for stop in stops:
                for product, qty in stop.demand.items():
                    capacity_utilization[product] = capacity_utilization.get(product, 0) + qty
        ...
        savings_percentage = ((unoptimized_estimate - total_distance) / unoptimized_estimate * 100) if unoptimized_estimate > 0 else 0
    ...
    async def _calculate_route_metrics(
        ...
        if self.google_routes_service:
            try:
                ...
                result = await self.google_routes_service._get_google_directions_enhanced(
                    depot, stops
                )
    ...
    async def _track_optimization_metrics(
        ...
        try:
            ...
            route_optimization_histogram.labels(
                method="VRP",
                num_stops=str(len(request.order_ids))
            ).observe(duration)
            ...
            if response.status == "success":
                vrp_solution_quality_gauge.labels(
                    metric_type="savings_percentage"
                ).set(response.savings_percentage)
                
                # Calculate average efficiency score
                if response.routes:
                    avg_efficiency = sum(r.efficiency_score for r in response.routes) / len(response.routes)
                    vrp_solution_quality_gauge.labels(
                        metric_type="efficiency_score"
                    ).set(avg_efficiency)
                
                # Calculate unassigned ratio
                unassigned_ratio = len(response.unassigned_orders) / len(request.order_ids) if request.order_ids else 0
                vrp_solution_quality_gauge.labels(
                    metric_type="unassigned_ratio"
                ).set(unassigned_ratio * 100)
