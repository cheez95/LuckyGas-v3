                ...
                async with session.post(url, data=data) as response:
                    ...
                    if response.status == 201:
                        return {
                            ...
                            "cost": float(result.get("price", 0)) * -30  # Convert USD to TWD estimate
                        }
                    else:
                        return {
                            "success": False,
        ...
        try:
            ...
            async with aiohttp.ClientSession(auth=auth) as session:
                async with session.get(url) as response:
                    ...
                    return {
                        "status": status_map.get(result["status"], NotificationStatus.PENDING),
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(api_url, params=params) as response:
                    ...
                    if len(parts) >= 5 and float(parts[1]) > 0:
                        ...
                        error_msg = error_codes.get(text.strip(), f"Unknown error: {text}")
                        return {
                            "success": False,
                            "error": error_msg
                        }
...
class MitakeProvider(SMSProviderBase):
    ...
    async def send_sms(self, phone: str, message: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            username = self.config.get("username")
            password = self.config.get("password")
            api_url = self.config.get("api_url", "https://api.mitake.com.tw/api/mtk/SmSend")
...
class EnhancedSMSService:
    ...
    async def _get_best_provider(self, db: AsyncSession) -> Optional[Tuple[SMSProvider, SMSProviderBase]]:
        ...
        result = await db.execute(
            select(ProviderConfig).where(
                ...
                ProviderConfig.priority.desc(),
                ProviderConfig.success_rate.desc()
            )
        )
    ...
    async def _check_rate_limit(self, provider: SMSProvider, limit: Optional[int]) -> bool:
    ...
    async def check_delivery_status(
        ...
        # Get SMS log
        result = await db.execute(
            select(SMSLog).where(SMSLog.id == message_id)
        )
    ...
    async def _update_template_stats(self, template_code: str, db: AsyncSession):
        ...
        if template:
            ...
            if template.sent_count > 0:
                template.effectiveness_score = template.delivered_count / template.sent_count
            await db.commit()
