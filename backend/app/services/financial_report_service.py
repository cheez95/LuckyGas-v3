...
from app.models import (
    Invoice, Payment, Customer, Order, InvoiceItem,
...
class FinancialReportService:
    ...
    async def get_revenue_summary(self, period: str) -> Dict[str, Any]:
        ...
        # Calculate summary
        gross_revenue = sum(inv.sales_amount for inv in invoices if inv.status == InvoiceStatus.ISSUED)
        tax_collected = sum(inv.tax_amount for inv in invoices if inv.status == InvoiceStatus.ISSUED)
        total_revenue = gross_revenue + tax_collected
        
        void_amount = sum(inv.total_amount for inv in invoices if inv.status == InvoiceStatus.VOID)
        
        # Payment collection
        collected = sum(inv.paid_amount for inv in invoices if inv.status == InvoiceStatus.ISSUED)
        outstanding = total_revenue - collected
        
        # By customer type
        b2b_revenue = sum(inv.total_amount for inv in invoices 
                         if inv.status == InvoiceStatus.ISSUED and inv.invoice_type == "B2B")
        b2c_revenue = sum(inv.total_amount for inv in invoices 
        ...
        return {
            ...
            "invoice_count": sum(1 for inv in invoices if inv.status == InvoiceStatus.ISSUED),
            "void_count": sum(1 for inv in invoices if inv.status == InvoiceStatus.VOID)
        }
    ...
    async def get_accounts_receivable_report(
        ...
        return {
            ...
            "customer_details": [
                {"customer_id": cid, **details}
                for cid, details in sorted_customers
            ]
        }
    ...
    async def get_cash_flow_report(
        ...
        return {
            ...
            "average_daily_inflow": sum(p.amount for p in payments) / ((date_to - date_from).days + 1)
        }
    ...
    async def get_customer_statement(
        ...
        # Get payments
        payment_query = select(Payment).join(Invoice).where(
            and_(
                Invoice.customer_id == customer_id,
                Payment.payment_date >= date_from,
                Payment.payment_date <= date_to
        ...
        # Get opening balance
        opening_balance = await self._get_customer_balance_before_date(customer_id, date_from)
    ...
    async def get_dashboard_metrics(self) -> Dict[str, Any]:
        ...
        current_month_revenue = await self._get_period_revenue(
            month_start, today
        )
    ...
    async def _get_customer_balance_before_date(
        ...
        payment_query = select(func.sum(Payment.amount)).join(Invoice).where(
            and_(
                Invoice.customer_id == customer_id,
