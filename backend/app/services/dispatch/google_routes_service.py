...
class GoogleRoutesService:
    ...
    async def calculate_distance_matrix(
        ...
        try:
            async with self.session.post(
                ...
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"Distance Matrix API error: {response.status} - {error_text}")
                ...
                return self._parse_distance_matrix(matrix, len(origins), len(destinations))
    ...
    def _parse_route_response(self, route: Dict[str, Any], request: RouteRequest) -> RouteResult:
