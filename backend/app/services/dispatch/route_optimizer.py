...
class RouteOptimizer:
    ...
    async def optimize_daily_routes(
        ...
        logger.info(f"Optimizing routes for {len(orders)} orders and {len(request.drivers)} drivers")
    ...
    async def _assign_clusters_to_drivers(
        ...
        for driver, constraints in drivers:
            if any(d[0].id == driver.id for d in assignments):
                ...
                remaining_capacity = self._calculate_remaining_capacity(
                    constraints.vehicle_capacity or VehicleCapacity(),
                    driver_orders
                )
            else:
                driver_orders = []
        ...
        if remaining_clusters:
            ...
            logger.warning(f"{unassigned_count} orders could not be assigned to any driver")
    ...
    async def _optimize_driver_route(
        ...
        try:
            ...
            end_time = start_time + timedelta(seconds=int(result.total_duration_seconds))
    ...
    def _can_fit_orders(
        ...
        if (cylinders.get("50kg", 0) > capacity.max_cylinders_50kg or
            cylinders.get("20kg", 0) > capacity.max_cylinders_20kg or
            cylinders.get("16kg", 0) > capacity.max_cylinders_16kg or
            cylinders.get("10kg", 0) > capacity.max_cylinders_10kg or
            cylinders.get("4kg", 0) > capacity.max_cylinders_4kg):
            return False
    ...
    def _calculate_remaining_capacity(
        self,
        initial_capacity: VehicleCapacity,
        orders: List[Order]
    ) -> VehicleCapacity:
    ...
    def _create_fallback_route(
        ...
        sorted_orders = sorted(
            orders,
            key=lambda o: (o.customer.area or "", o.customer.address)
        )
