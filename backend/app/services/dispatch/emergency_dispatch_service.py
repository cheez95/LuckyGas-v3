...
class EmergencyDispatchService:
    ...
    async def create_emergency_dispatch(
        ...
        try:
            ...
            valid_types = ['gas_leak', 'urgent_delivery', 'customer_emergency', 'driver_emergency']
            if emergency_data.get('type') not in valid_types:
                raise ValidationException(f"Invalid emergency type: {emergency_data.get('type')}")
            ...
            return {
                ...
                'assignedDriverName': driver.full_name if emergency_data.get('driverId') else None
            }
    ...
    async def get_emergency_queue(
        ...
        """
        query = select(Order).join(Customer).where(
        ...
        for order in orders:
            ...
            emergency_type = notes_parts[0].replace('EMERGENCY: ', '') if notes_parts else 'urgent_delivery'
            description = notes_parts[1] if len(notes_parts) > 1 else order.delivery_notes
            ...
            emergency_queue.append({
                ...
                } if order.customer.latitude and order.customer.longitude else None
            })
    ...
    async def get_emergency_statistics(
        ...
        response_time_query = select(
            ...
            RouteStop, Order.id == RouteStop.order_id
        ).join(
            RoutePlan, RouteStop.route_plan_id == RoutePlan.id
        ).where(
            Order.is_urgent == True,
            Order.delivery_notes.like('EMERGENCY:%'),
            Order.created_at >= date_from,
