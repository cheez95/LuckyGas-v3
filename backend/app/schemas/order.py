...
class OrderBase(BaseModel):
    customer_id: int = Field(..., alias="客戶編號", description="Customer ID", gt=0)
    scheduled_date: datetime = Field(
        ..., alias="預定配送日期", description="Scheduled delivery date"
    )
    ...
    # Cylinder quantities
    qty_50kg: int = Field(
        0, alias="50公斤數量", description="Quantity of 50kg cylinders", ge=0, le=100
    )
    qty_20kg: int = Field(
        0, alias="20公斤數量", description="Quantity of 20kg cylinders", ge=0, le=100
    )
    qty_16kg: int = Field(
        0, alias="16公斤數量", description="Quantity of 16kg cylinders", ge=0, le=100
    )
    qty_10kg: int = Field(
        0, alias="10公斤數量", description="Quantity of 10kg cylinders", ge=0, le=100
    )
    qty_4kg: int = Field(
        0, alias="4公斤數量", description="Quantity of 4kg cylinders", ge=0, le=100
    )
    ...
    is_urgent: bool = Field(
        False, alias="緊急訂單", description="Whether this is an urgent order"
    )
    ...
    def validate_order(self):
        ...
        total_qty = (
            self.qty_50kg + self.qty_20kg + self.qty_16kg + self.qty_10kg + self.qty_4kg
        )
        if total_qty == 0:
...
class Order(OrderBase):
    ...
    payment_status: PaymentStatus = Field(
        ..., alias="付款狀態", description="Payment status"
    )

    # Pricing
    total_amount: float = Field(
        ..., alias="總金額", description="Total amount before discount", ge=0
    )
    discount_amount: float = Field(
        0, alias="折扣金額", description="Discount amount", ge=0
    )
    final_amount: float = Field(
        ..., alias="應付金額", description="Final amount after discount", ge=0
    )
    ...
    delivered_at: Optional[datetime] = Field(
        None, alias="配送時間", description="Delivery completion timestamp"
    )

    # Additional fields
    driver_id: Optional[int] = Field(
        None, alias="司機編號", description="Assigned driver ID"
    )
    route_id: Optional[int] = Field(
        None, alias="路線編號", description="Assigned route ID"
    )
...
class OrderBaseV2(BaseModel):
    ...
    _validate_time = field_validator("delivery_time_start", "delivery_time_end")(
        OrderBase.validate_time_format
    )
    _validate_address = field_validator("delivery_address")(
        OrderBase.validate_delivery_address
    )
    _validate_date = field_validator("scheduled_date")(
        OrderBase.validate_scheduled_date
    )
