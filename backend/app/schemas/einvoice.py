...
class InvoiceData(BaseModel):
    ...
    InvoiceNo: str = Field(..., regex=r'^[A-Z]{2}\d{8}$', description="Invoice number (e.g., AB12345678)")
    InvoiceDate: str = Field(..., regex=r'^\d{4}/\d{2}/\d{2}$', description="Invoice date (YYYY/MM/DD)")
    InvoiceTime: str = Field(..., regex=r'^\d{2}:\d{2}:\d{2}$', description="Invoice time (HH:MM:SS)")
    ...
    # Buyer Information
    BuyerId: Optional[str] = Field(None, regex=r'^\d{8}$', description="Buyer tax ID (8 digits)")
    ...
    ItemDescription: List[str] = Field(..., min_length=1, description="Item descriptions")
    ...
    @field_validator('ItemCount', 'ItemDescription', 'ItemQuantity', 'ItemUnit', 'ItemUnitPrice', 'ItemAmount', 'ItemTaxType')
...
class InvoiceVoidRequest(BaseModel):
    ...
    invoice_number: str = Field(..., regex=r'^[A-Z]{2}\d{8}$', description="Invoice number to void")
    reason: str = Field(..., max_length=200, description="Void reason")


class InvoiceAllowanceRequest(BaseModel):
    """Request to issue allowance"""
    original_invoice_number: str = Field(..., regex=r'^[A-Z]{2}\d{8}$', description="Original invoice number")
    allowance_amount: float = Field(..., gt=0, description="Allowance amount before tax")
