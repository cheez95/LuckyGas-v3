...
class APIKeyCreate(BaseModel):
    ...
    tier: str = Field("basic", description="Rate limiting tier (basic, standard, premium, enterprise)")
...
class APIKeyResponse(BaseModel):
    ...
    rate_limits: Dict[str, Any] = Field(..., description="Rate limit configuration for this tier")
...
class APIKeyListResponse(BaseModel):
    ...
    usage_count: int = Field(0, description="Total number of requests made with this key")
...
class RateLimitStatus(BaseModel):
    ...
    retry_after: Optional[int] = Field(None, description="Seconds to wait before retry (when rate limited)")
