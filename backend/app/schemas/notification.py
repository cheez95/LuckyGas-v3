...
class SMSSendRequest(BaseModel):
    ...
    message: Optional[str] = Field(None, description="Message content (if not using template)")
    ...
    template_data: Optional[Dict[str, Any]] = Field(None, description="Data for template placeholders")
    ...
    def validate_phone(cls, v):
        ...
        if not (
            (cleaned.startswith("09") and len(cleaned) == 10) or  # Mobile
            (cleaned.startswith("0") and len(cleaned) in [9, 10]) or  # Landline
            (cleaned.startswith("+886") and len(cleaned) in [12, 13])  # International
        ):
...
class SMSBulkSendRequest(BaseModel):
    ...
    batch_size: Optional[int] = Field(100, ge=1, le=500, description="Batch size for sending")
...
class ProviderConfigBase(BaseModel):
    ...
    cost_per_segment: Optional[float] = Field(None, description="Additional segment cost")
