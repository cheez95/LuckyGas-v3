...
class RoutePlan(Base):
    ...
    # Relationships
    driver = relationship("User", foreign_keys=[driver_id], back_populates="assigned_routes")
    stops = relationship("RouteStop", back_populates="route_plan", cascade="all, delete-orphan")
...
class RouteStop(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    route_plan_id = Column(Integer, ForeignKey("route_plans.id"), nullable=False, index=True)
    order_id = Column(Integer, ForeignKey("orders.id"), nullable=False, index=True)
    
    # Sequence and status
    stop_sequence = Column(Integer, nullable=False)  # Order of stops
    stop_status = Column(SQLEnum(StopStatus), default=StopStatus.PENDING, nullable=False)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint("route_plan_id", "stop_sequence", name="uq_route_stop_sequence"),
        Index("idx_stop_status", "stop_status"),
        Index("idx_order_route", "order_id", "route_plan_id"),
    )
    
    def __repr__(self):
        return f"<RouteStop {self.route_plan_id}-{self.stop_sequence}>"
...
class DriverAssignment(Base):
    ...
    assignment_type = Column(String(50), default="regular")  # regular, emergency, substitute
