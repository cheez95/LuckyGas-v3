...
class InvoiceSequence(Base):
    ...
    # Period and prefix
    year_month = Column(String(6), nullable=False, comment='YYYYMM format (e.g., 202501)')
    prefix = Column(String(2), nullable=False, comment='Invoice prefix (e.g., AA, AB)')
    
    # Allocated range
    range_start = Column(Integer, nullable=False, comment='Starting number of allocated range')
    range_end = Column(Integer, nullable=False, comment='Ending number of allocated range')
    
    # Current sequence
    current_number = Column(Integer, nullable=False, comment='Current sequential number')
