...
from sqlalchemy import (
    ...
    Float, Index, UniqueConstraint, ForeignKey, Text, Table
)
...
class FeatureFlag(Base):
    ...
    status = Column(Enum(FeatureFlagStatus), nullable=False, default=FeatureFlagStatus.INACTIVE)
    ...
    variants = relationship("FeatureFlagVariant", back_populates="feature_flag", cascade="all, delete-orphan")
    audit_logs = relationship("FeatureFlagAudit", back_populates="feature_flag", cascade="all, delete-orphan")
    ...
    def __repr__(self):
        return f"<FeatureFlag(name={self.name}, type={self.type}, status={self.status})>"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_customers_count": len(self.enabled_customers) if self.enabled_customers else 0,
            "disabled_customers_count": len(self.disabled_customers) if self.disabled_customers else 0,
