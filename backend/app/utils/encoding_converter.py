...
class Big5ToUTF8Converter:
        ...
        if isinstance(text, bytes):
            if not source_encoding:
                ...
                self.conversion_stats["encoding_detected"][source_encoding] = (
                    self.conversion_stats["encoding_detected"].get(source_encoding, 0)
                    + 1
                )
    ...
    def convert_sqlite_table(
        ...
        if output_path:
            if output_path.endswith(".csv"):
                ...
                df_converted.to_sql(
                    table_name, conn_out, if_exists="replace", index=False
                )
                conn_out.close()
    ...
    def get_conversion_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.conversion_stats["successful"]
                / self.conversion_stats["total_processed"]
                if self.conversion_stats["total_processed"] > 0
                else 0
            )
            * 100,
        }
...
def convert_customer_data(db_path: str, output_dir: str) -> Dict[str, pd.DataFrame]:
    ...
    for table_name, text_columns in tables_config.items():
        ...
        try:
            ...
            logger.info(
                f"Table {table_name} conversion complete. Success rate: {stats['success_rate']:.2f}%"
            )

            if stats["errors"]:
                logger.warning(
                    f"Errors encountered in {table_name}: {len(stats['errors'])}"
                )
