...
class SecurityAudit:
    ...
    async def _check_alert_threshold(event_type: str):
        ...
        if event_type == "failed_login" and count >= config.alert_failed_logins:
            alert_needed = True
        elif event_type == "permission_denied" and count >= config.alert_permission_denied:
            alert_needed = True
        elif event_type == "suspicious_activity" and count >= config.alert_suspicious_requests:
            alert_needed = True
...
class SecureHeaders:
    ...
    def add_cache_headers(response, cache_time: int = 0, private: bool = True):
        ...
        if cache_time > 0:
            cache_control = f"private, max-age={cache_time}" if private else f"public, max-age={cache_time}"
        else:
