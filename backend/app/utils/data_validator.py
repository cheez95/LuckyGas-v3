...
class DataValidator:
    ...
    def validate_customer_data(self, customer_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in customer_df.iterrows():
            ...
            # Validate address
            if row.get('address'):
                addr_valid, addr_issues = self.validate_taiwan_address(str(row['address']))
                if not addr_valid:
            ...
            if pd.notna(row.get('latitude')) and pd.notna(row.get('longitude')):
                ...
                if not (21.5 <= lat <= 26 and 119 <= lng <= 122.5):
                    ...
                    self.validation_stats['warnings_by_type']['invalid_coordinates'] += 1
            
            # Check cylinder inventory
            cylinder_cols = ['cylinder_50kg', 'cylinder_20kg', 'cylinder_16kg', 
                           'cylinder_10kg', 'cylinder_4kg']
                ...
                if pd.notna(row.get(col)):
                    try:
                        ...
                        if value < 0:
                            ...
                            self.validation_stats['errors_by_type']['negative_inventory'] += 1
                        elif value > 1000:
                            warnings.append(f"Unusually high {col}: {value}")
                            self.validation_stats['warnings_by_type']['high_inventory'] += 1
                    except:
            ...
            if row.get('needs_same_day_delivery') and not any(
                ...
                warnings.append("Same-day delivery requested but no time slots selected")
    ...
    def validate_order_data(self, order_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in order_df.iterrows():
            ...
            # Validate status
            if row.get('status'):
                valid_statuses = ['pending', 'confirmed', 'in_delivery', 'delivered', 'cancelled', 'failed']
            ...
            # Validate quantities
            delivered_cols = ['delivered_50kg', 'delivered_20kg', 'delivered_16kg', 
                            'delivered_10kg', 'delivered_4kg']
            returned_cols = ['returned_50kg', 'returned_20kg', 'returned_16kg', 
                           'returned_10kg', 'returned_4kg']
            ...
            for col in delivered_cols:
                if pd.notna(row.get(col)):
                    try:
                        ...
                        if value < 0:
                            errors.append(f"Negative {col}: {value}")
                            self.validation_stats['errors_by_type']['negative_quantity'] += 1
                ...
                if pd.notna(row.get(ret_col)) and pd.notna(row.get(del_col)):
                    try:
                        ...
                        if returned > delivered:
                            ...
                            self.validation_stats['warnings_by_type']['return_exceed'] += 1
                    except:
                        pass
            ...
            if str(row.get('status')).lower() == 'delivered':
                if pd.isna(row.get('actual_delivery_time')):
                    ...
                    self.validation_stats['warnings_by_type']['missing_delivery_time'] += 1
            ...
            if errors:
                self.validation_errors.append({
                    'record': f"order_{row.get('id', idx)}",
                    'errors': errors
                })
                self.validation_stats['invalid_records'] += 1
            else:
    ...
    def generate_validation_report(self) -> str:
        ...
        if self.validation_stats['total_records'] > 0:
            validity_rate = (self.validation_stats['valid_records'] / 
        ...
        if self.validation_stats['errors_by_type']:
            ...
            report.append("-" * 40)
            for error_type, count in sorted(self.validation_stats['errors_by_type'].items(), 
        ...
        if self.validation_stats['warnings_by_type']:
            ...
            report.append("-" * 40)
            for warning_type, count in sorted(self.validation_stats['warnings_by_type'].items(), 
                                            key=lambda x: x[1], reverse=True):
...
def validate_migration_data(
    ...
    validated_customers.to_csv(
        os.path.join(output_dir, "validated_customers.csv"),
        index=False
    )
    ...
    validated_orders.to_csv(
        os.path.join(output_dir, "validated_orders.csv"),
        index=False
    )
    ...
    summary = {
        ...
        'overall_validity_rate': (
            ...
            (customer_stats['total_records'] + order_stats['total_records']) * 100
        )
    }
...
if __name__ == "__main__":
    ...
    print(f"\nValidation Summary:")
    print(f"  Valid Customers: {results['valid_customers']}/{results['total_customers']}")
