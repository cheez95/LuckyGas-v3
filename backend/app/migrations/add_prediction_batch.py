...
def upgrade():
    ...
    op.create_index(
        op.f("ix_prediction_batches_id"), "prediction_batches", ["id"], unique=False
    )

    # Add batch_id to delivery_predictions
    op.add_column(
        "delivery_predictions", sa.Column("batch_id", sa.Integer(), nullable=True)
    )
    op.create_index(
        op.f("ix_delivery_predictions_batch_id"),
...
def downgrade():
    ...
    op.drop_constraint(
        "fk_delivery_predictions_batch_id", "delivery_predictions", type_="foreignkey"
    )
    op.drop_index(
        op.f("ix_delivery_predictions_batch_id"), table_name="delivery_predictions"
    )
