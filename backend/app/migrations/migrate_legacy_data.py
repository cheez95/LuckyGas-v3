...
class LegacyDataMigrator:
        ...
        for product_data in standard_products:
            ...
            result = await session.execute(
                select(GasProduct).where(
                    GasProduct.product_code == product_data["code"]
                )
            )
            ...
            if not existing:
                product = GasProduct(
                    ...
                    deposit_amount=product_data["size"]
                    * 60,  # Rough deposit calculation
                    is_available=True,
                    category=(
                        "standard" if "BIZ" not in product_data["code"] else "business"
                    ),
                )
                session.add(product)
            ...
            for _, row in batch.iterrows():
                try:
                    ...
                    if not validate_taiwan_data(row["address"], "address"):
                        logger.warning(
                            f"Invalid address for customer {row['client_code']}: {row['address']}"
                        )
                    ...
                    customer = Customer(
                        ...
                        tax_id=(
                            row.get("tax_id")
                            if validate_taiwan_data(row.get("tax_id", ""), "tax_id")
                            else None
                        ),
                        customer_type=(
                            "corporate" if row.get("is_corporate") else "residential"
                        ),
                        ...
                        needs_same_day_delivery=bool(
                            row.get("needs_same_day_delivery")
                        ),
                        # Equipment
                        ...
                        # Usage statistics
                        monthly_delivery_volume=float(
                            row.get("monthly_delivery_volume", 0)
                        ),
                    ...
                    logger.error(
                        f"Failed to migrate customer {row['client_code']}: {e}"
                    )
    ...
    async def migrate_drivers(self, session: AsyncSession) -> Dict[int, int]:
        ...
        for _, row in df.iterrows():
            try:
                ...
                self.errors.append(
                    {"table": "drivers", "record": row["employee_id"], "error": str(e)}
                )
            ...
            try:
                ...
                vehicle = Vehicle(
                    ...
                    last_maintenance_date=(
                        pd.to_datetime(row.get("last_maintenance"))
                        if pd.notna(row.get("last_maintenance"))
                        else None
                    ),
                    next_maintenance_date=(
                        pd.to_datetime(row.get("next_maintenance"))
                        if pd.notna(row.get("next_maintenance"))
                        else None
                    ),
                    created_at=pd.to_datetime(row.get("created_at", datetime.now())),
                    updated_at=pd.to_datetime(row.get("updated_at", datetime.now())),
            ...
            for _, row in batch.iterrows():
                try:
                    ...
                    order = Order(
                        ...
                        delivery_time_start=self._parse_time(
                            row.get("scheduled_time_start")
                        ),
                        delivery_time_end=self._parse_time(
                            row.get("scheduled_time_end")
                        ),
                        # Status mapping
                        ...
                        driver_id=driver_id,
                        route_sequence=(
                            int(row.get("route_sequence", 0))
                            if pd.notna(row.get("route_sequence"))
                            else None
                        ),
                        estimated_distance=(
                        ...
                        estimated_duration=(
                            int(row.get("estimated_duration_minutes", 0))
                            if pd.notna(row.get("estimated_duration_minutes"))
                            else None
                        ),
                        # Delivery confirmation
                        delivered_at=(
                            pd.to_datetime(row["actual_delivery_time"])
                            if pd.notna(row.get("actual_delivery_time"))
                            else None
                        ),
                    ...
                    total_amount = await self._create_order_items(
                        session, order.id, row
                    )
    ...
    async def _create_order_items(
        self, session: AsyncSession, order_id: int, delivery_row: pd.Series
    ) -> float:
        """
        ...
        for delivered_col, returned_col, product_code, unit_price in cylinder_mappings:
            delivered_qty = (
                int(delivery_row.get(delivered_col, 0))
                if pd.notna(delivery_row.get(delivered_col))
                else 0
            )
            returned_qty = (
                int(delivery_row.get(returned_col, 0))
                if pd.notna(delivery_row.get(returned_col))
                else 0
            )
            ...
            if delivered_qty > 0:
                ...
                if product:
                    order_item = OrderItem(
                        ...
                        final_amount=delivered_qty
                        * unit_price,  # No discount in legacy
                    )
    ...
    async def generate_migration_report(self):
        ...
        with open(report_path, "w", encoding="utf - 8") as f:
            ...
            for table, stats in self.stats.items():
                success_rate = (
                    (stats["migrated"] / stats["total"] * 100)
                    if stats["total"] > 0
                    else 0
                )
            ...
            f.write(
                f"Total strings processed: {conv_report['statistics']['total_processed']}\n"
            )
            f.write(
                f"Successful conversions: {conv_report['statistics']['successful']}\n"
            )
            f.write(f"Failed conversions: {conv_report['statistics']['failed']}\n")
            f.write(
                f"Characters replaced: {conv_report['statistics']['replaced_chars']}\n"
            )
