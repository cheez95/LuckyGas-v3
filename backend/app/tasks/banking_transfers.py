...
from app.models.banking import (
    PaymentBatch, BankConfiguration, PaymentBatchStatus,
    ReconciliationStatus, ReconciliationLog
)
        ...
        for bank_config in banks:
            try:
                ...
                if not _is_within_processing_window(bank_config):
                    logger.info(f"Outside processing window for {bank_config.bank_code}")
                    continue
                ...
                if batch.total_transactions == 0:
                    logger.info(f"No transactions to process for {bank_config.bank_code}")
                    continue
                
                # Generate ACH file
                file_content = ach_generator.generate_payment_file(
                    batch,
                    encoding=bank_config.encoding or 'big5'
                )
...
def check_and_process_reconciliation(self) -> Dict[str, Any]:
    ...
    try:
        ...
        for bank_config in banks:
            try:
                logger.info(f"Checking reconciliation files for {bank_config.bank_code}")
...
def process_retry_queue() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'processed': processed,
            'remaining': sftp_service._retry_queue.qsize()
        }
...
def generate_daily_report() -> Dict[str, Any]:
    ...
    try:
        ...
        reconciliations = db.query(ReconciliationLog).filter(
        ...
        report = {
            ...
            'payment_batches': {
                'total': len(batches),
                'uploaded': len([b for b in batches if b.status == PaymentBatchStatus.UPLOADED]),
                'failed': len([b for b in batches if b.status == PaymentBatchStatus.FAILED]),
            ...
            'reconciliations': {
                ...
                'unmatched_transactions': sum(r.unmatched_records for r in reconciliations),
...
def perform_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        if health['status'] == 'degraded':
            ...
            notification_service = NotificationService(db)
            failed_banks = [
                check['bank'] for check in health['checks'] 
