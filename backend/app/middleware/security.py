...
class SuspiciousActivityDetector:
    ...
    def __init__(self):
        self.suspicious_patterns = {
            ...
            "auth_failures": {"threshold": 5, "window": 300},  # 5 failed logins in 5 minutes
            "scanning": {"threshold": 20, "window": 60},  # 20 different endpoints in 60 seconds
            "large_requests": {"threshold": 5, "window": 60},  # 5 large requests in 60 seconds
        }
    
    async def track_activity(self, client_id: str, activity_type: str, details: Dict = None):
        ...
        try:
            ...
            if count >= pattern["threshold"]:
                await self._handle_suspicious_activity(client_id, activity_type, count, details)
                
        except Exception as e:
            logger.error(f"Activity tracking error: {str(e)}", exc_info=True)
    
    async def _handle_suspicious_activity(self, client_id: str, activity_type: str, count: int, details: Dict = None):
    ...
    async def _log_security_event(self, client_id: str, event_type: str, count: int, details: Dict = None):
        ...
        await cache.set(key, json.dumps(events_list), expire=86400 * 7)  # Keep for 7 days
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_request(self, request: Request, client_id: str) -> Optional[Response]:
        """Validate incoming request for security threats."""
        
        # Check URL length
        if len(str(request.url)) > self.MAX_URL_LENGTH:
            await self.detector.track_activity(client_id, "scanning", {"reason": "long_url"})
            return JSONResponse(
                status_code=414,
                content={"detail": "請求 URL 過長"}
            )
        
        # Check headers size
        total_header_size = sum(len(k) + len(v) for k, v in request.headers.items())
        if total_header_size > self.MAX_HEADER_SIZE:
            await self.detector.track_activity(client_id, "large_requests", {"reason": "large_headers"})
            return JSONResponse(
                status_code=431,
                content={"detail": "請求標頭過大"}
            )
        
        # Check for path traversal in URL
        if self.validator.is_path_traversal_attempt(str(request.url)):
            await self.detector.track_activity(client_id, "scanning", {"reason": "path_traversal"})
            logger.warning(f"Path traversal attempt from {client_id}: {request.url}")
            return JSONResponse(
                status_code=400,
                content={"detail": "無效的請求路徑"}
            )
        ...
        for param_name, param_value in request.query_params.items():
            if isinstance(param_value, str):
                ...
                if self.validator.is_sql_injection_attempt(param_value):
                    ...
                    logger.warning(f"SQL injection attempt from {client_id}: {param_name}={param_value}")
                    return JSONResponse(
                        status_code=400,
                        content={"detail": "無效的請求參數"}
                    )
                ...
                if self.validator.is_xss_attempt(param_value):
                    await self.detector.track_activity(
                        client_id,
                        "scanning",
                        {"reason": "xss", "param": param_name}
                    )
                    logger.warning(f"XSS attempt from {client_id}: {param_name}={param_value}")
                    return JSONResponse(
                        status_code=400,
                        content={"detail": "無效的請求參數"}
                    )
    ...
    async def _audit_log_request(self, request: Request, response: Response, start_time: datetime):
        ...
        if request.method != "GET" or response.status_code >= 400:
            ...
            try:
                ...
                await cache.set(audit_key, json.dumps(logs_list), expire=86400 * 30)  # Keep 30 days
            except Exception as e:
...
__all__ = ["SecurityMiddleware", "SecurityHeaders", "SecurityValidation", "CSRFProtection"]