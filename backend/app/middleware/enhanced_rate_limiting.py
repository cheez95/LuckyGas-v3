...
class APIKeyManager:
    ...
    @staticmethod
    async def generate_api_key(user_id: int, tier: str = "basic", name: str = "") -> Dict[str, Any]:
...
class EnhancedRateLimitMiddleware:
    ...
    @classmethod
    def get_endpoint_limits(cls, path: str, user_role: Optional[str] = None) -> List[str]:
...
async def get_api_key_tier(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Optional[str]:
