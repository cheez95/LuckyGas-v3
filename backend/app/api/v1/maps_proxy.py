...
class GoogleMapsProxy:
    ...
    def http_client(self) -> httpx.AsyncClient:
        ...
        if not self._http_client:
            self._http_client = httpx.AsyncClient(
                timeout=30.0,
                headers={"User-Agent": "LuckyGas-Backend/1.0"}
            )
    ...
    async def make_api_call(
        ...
        async with self._request_lock:
            ...
            future = asyncio.create_task(self._execute_api_call(service, endpoint, params))
...
async def distance_matrix(
    ...
    destinations: str = Query(..., description="Destination locations (pipe-separated)"),
    mode: str = Query("driving", description="Travel mode"),
    departure_time: Optional[str] = Query(None, description="Departure time"),
    language: str = Query("zh-TW", description="Response language"),
    current_user: User = Depends(get_current_active_user),
