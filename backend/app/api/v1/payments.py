...
async def create_payment(
    ...
    if payment_data.amount > remaining_amount:
        raise HTTPException(
            status_code=400, detail=f"付款金額超過未付餘額 (餘額: {remaining_amount})"
        )

    # Create payment using service
    service = PaymentService(db)
    payment = await service.create_payment(
        payment_data=payment_data, created_by=current_user.id
    )
...
async def get_payment(
    ...
    payment = await db.get(Payment, payment_id, options=[selectinload(Payment.invoice)])
...
async def update_payment(
    ...
    updated_payment = await service.update_payment(
        payment_id=payment_id, payment_update=payment_update
    )
...
async def reconcile_payments(
    ...
    service = PaymentService(db)
    result = await service.reconcile_payments(date_from=date_from, date_to=date_to)
