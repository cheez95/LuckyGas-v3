...
from app.api.auth_deps.auth import get_current_user, require_roles
...
async def create_api_key(
    ...
    _: None = Depends(require_roles(["super_admin", "admin", "manager"]))
) -> APIKeyResponse:
    """
    ...
    try:
        ...
        if key_data.tier in ["premium", "enterprise"] and current_user.role not in ["super_admin", "admin"]:
            raise HTTPException(
...
async def get_api_key_tiers(
    ...
    for tier, info in APIKeyManager.TIERS.items():
        if tier in ["premium", "enterprise"] and current_user.role not in ["super_admin", "admin"]:
            continue
    ...
    return {
        ...
        "current_user_limits": {
            "rate_limit": "200/hour" if current_user.role in ["admin", "super_admin"] else "100/hour",
            "burst_limit": "40/minute" if current_user.role in ["admin", "super_admin"] else "20/minute"
        }
    }
...
async def get_api_key_usage(
    ...
    try:
        ...
        current_hour_key = f"rate_limit:api_key:{current_user.id}:{key_info['tier']}:hour"
        current_minute_key = f"rate_limit:api_key:{current_user.id}:{key_info['tier']}:minute"
