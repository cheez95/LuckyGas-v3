...
from app.schemas.banking import (
    ...
    PaymentBatchCreate, PaymentBatchResponse, PaymentBatchListResponse,
    GeneratePaymentFileRequest, GeneratePaymentFileResponse,
    UploadFileRequest, UploadFileResponse,
    CheckReconciliationResponse, ProcessReconciliationRequest,
    PaymentStatusReport, ReconciliationLogResponse
)
...
async def upload_payment_file(
    *,
    ...
    bank_config = db.query(BankConfiguration).filter_by(
        bank_code=bank_code,
        is_active=True
    ).first()
    
    if not bank_config:
        raise HTTPException(status_code=404, detail=f"No active configuration for bank {bank_code}")
    ...
    *,
    ...
    for bank in banks:
        try:
            ...
            results.append(CheckReconciliationResponse(
                bank_code=bank.bank_code,
                new_files=new_files,
                checked_at=datetime.utcnow()
            ))
        except Exception as e:
            logger.error(f"Failed to check reconciliation for {bank.bank_code}: {str(e)}")
            results.append(CheckReconciliationResponse(
                bank_code=bank.bank_code,
                new_files=[],
                checked_at=datetime.utcnow()
...
async def list_payment_batches(
    *,
    ...
    batches = query.order_by(PaymentBatch.created_at.desc()).offset(skip).limit(limit).all()
...
async def create_bank_configuration(
    *,
    ...
    existing = db.query(BankConfiguration).filter_by(bank_code=bank_in.bank_code).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Bank code {bank_in.bank_code} already exists")
...
async def update_bank_configuration(
    *,
    ...
    if 'sftp_password' in update_data and update_data['sftp_password']:
        update_data['sftp_password'] = update_data['sftp_password']  # TODO: Encrypt this
