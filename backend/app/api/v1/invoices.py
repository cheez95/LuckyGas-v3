...
from app.schemas.invoice import (
    ...
    InvoiceItemCreate, InvoiceSearchParams,
    InvoiceStats, InvoiceBulkAction
)
...
async def get_invoice(
    ...
    invoice = await db.get(Invoice, invoice_id, options=[
        selectinload(Invoice.customer),
        selectinload(Invoice.items),
        selectinload(Invoice.order),
...
async def update_invoice(
    ...
    if invoice.status != InvoiceStatus.DRAFT and not current_user.role == "super_admin":
        raise HTTPException(
            status_code=400, 
            detail="只有草稿發票可以修改，已開立發票請使用作廢或折讓"
        )
...
async def issue_invoice(
    ...
    try:
        issued_invoice = await service.issue_invoice(
            invoice_id=invoice_id,
            einvoice_service=einvoice_service
        )
...
async def bulk_invoice_action(
    ...
    if action.action == "issue":
        ...
        results = await service.bulk_issue_invoices(
            invoice_ids=action.invoice_ids,
            einvoice_service=einvoice_service
        )
