...
from app.models.gas_product import GasProduct as GasProductModel, DeliveryMethod, ProductAttribute
from app.schemas.gas_product import GasProduct, GasProductCreate, GasProductUpdate, GasProductList
...
async def get_products(
    ...
    # Apply filters
    if delivery_method:
        base_query = base_query.where(GasProductModel.delivery_method == delivery_method)
    ...
    if delivery_method:
        count_query = count_query.where(GasProductModel.delivery_method == delivery_method)
    if size_kg:
...
async def get_available_products(
    ...
    # Get total count
    count_query = select(func.count()).select_from(GasProductModel).where(
        GasProductModel.is_available == True,
        GasProductModel.is_active == True
    )
    if delivery_method:
