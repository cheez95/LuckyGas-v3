...
async def get_migration_metrics(
    ...
    try:
        ...
        avg_times = [
            ...
            for m in sync_metrics.values()
            if m.average_sync_time_ms > 0
        ]
            ...
            operations.append(
                {
                    ...
                    "completedAt": (
                        (datetime.utcnow() - timedelta(minutes=i * 5 - 2)).isoformat()
                        if op_status == SyncStatus.COMPLETED
                        else None
                    ),
                    "errorMessage": (
                        "Connection timeout" if op_status == SyncStatus.FAILED else None
                    ),
...
async def select_customers(
    ...
    try:
        ...
        if request.minOrderCount:
            ...
            customers = [
                c for c in customers if c.total_orders >= request.minOrderCount
            ]
        ...
        return CustomerSelectionResponse(
            customerIds=customer_ids, count=len(customer_ids)
        )
        ...
        for i in range(0, len(request.customerIds), request.batchSize):
            ...
            for customer_id in batch:
                ...
                await db.execute(
                    ...
                    .values(in_pilot_program=True, pilot_enrolled_at=datetime.utcnow())
                )
