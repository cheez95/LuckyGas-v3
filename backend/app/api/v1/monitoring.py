...
@router.post("/performance/baseline", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/performance/slow-queries", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/performance/response-times", dependencies=[Depends(get_current_active_superuser)])
...
@router.post("/performance/alert-thresholds", dependencies=[Depends(get_current_active_superuser)])
async def update_alert_thresholds(
    ...
    if invalid:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid threshold keys: {invalid}"
        )
