...
from app.models.notification import (
    ...
    NotificationStatus, SMSProvider, NotificationChannel
)
from app.schemas.notification import (
    SMSSendRequest, SMSSendResponse, SMSBulkSendRequest,
    SMSStatusResponse, SMSTemplateCreate, SMSTemplateUpdate,
    SMSTemplateResponse, NotificationLogResponse,
    ProviderConfigCreate, ProviderConfigUpdate, ProviderConfigResponse,
    SMSLogResponse, NotificationStatsResponse
)
    ...
    try:
        result = await enhanced_sms_service.send_sms(
            ...
            metadata={
                ...
                **request.metadata
            } if request.metadata else {"sent_by": current_user.id},
            db=db
        )
...
async def delete_sms_template(
    ...
    if current_user.role != UserRole.SUPER_ADMIN:
        raise HTTPException(status_code=403, detail="Only super admin can delete templates")
...
async def create_provider_config(
    ...
    # Check permissions
    if current_user.role != UserRole.SUPER_ADMIN:
        raise HTTPException(status_code=403, detail="Only super admin can manage providers")
...
async def get_notification_stats(
    ...
    sms_query = select(
        ...
        func.sum(func.cast(SMSLog.status == NotificationStatus.SENT, type_=int)).label("sent"),
        func.sum(func.cast(SMSLog.status == NotificationStatus.DELIVERED, type_=int)).label("delivered"),
        func.sum(func.cast(SMSLog.status == NotificationStatus.FAILED, type_=int)).label("failed"),
        func.sum(SMSLog.cost).label("total_cost"),
        func.sum(SMSLog.segments).label("total_segments")
    ).where(
        and_(
            SMSLog.created_at >= start_date,
            SMSLog.created_at <= end_date
        )
    )
    ...
    return NotificationStatsResponse(
        ...
        sms_stats={
            ...
            "delivery_rate": (sms_stats.delivered / sms_stats.total * 100) if sms_stats.total else 0
        },
