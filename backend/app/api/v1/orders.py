...
async def create_order_v2(
    ...
    query = (
        select(Order)
        .options(selectinload(Order.order_items).selectinload(OrderItem.gas_product))
...
async def get_order_stats(
    ...
    stats = await order_service.get_order_statistics(
        start_date=date_from.date(), end_date=date_to.date(), area=None
    )
        ...
        order_dict = {
            ...
            "items": (
                [
                    {
                        "product_name": (
                            item.gas_product.display_name if item.gas_product else None
                        ),
...
async def unblock_customer_credit(
    ...
    if current_user.role not in ["super_admin", "manager"]:
        raise HTTPException(
            status_code=403, detail="只有經理以上權限才能解除信用額度封鎖"
        )
