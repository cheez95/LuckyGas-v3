...
async def get_today_routes(
    ...
    stmt = select(Route).where(
        ...
        selectinload(Route.deliveries).selectinload(RouteDelivery.order).selectinload(Order.customer),
        selectinload(Route.deliveries).selectinload(RouteDelivery.order).selectinload(Order.items).selectinload(OrderItem.product)
    )
    ...
    for route in routes:
        ...
        completed_deliveries = sum(1 for d in route.deliveries if d.status == DeliveryStatus.DELIVERED)
        ...
        route_responses.append(RouteListResponse(
            id=str(route.id),
...
async def get_today_stats(
    ...
    stmt = select(RouteDelivery).join(Route).where(
        and_(
            Route.driver_id == current_user.id,
            Route.scheduled_date >= today,
            Route.scheduled_date < tomorrow,
...
async def get_route_details(
    ...
    # Build delivery list
    deliveries = []
    for idx, route_delivery in enumerate(sorted(route.deliveries, key=lambda d: d.sequence)):
...
async def update_delivery_status(
    ...
    stmt = select(RouteDelivery).join(Route).where(
        ...
        selectinload(RouteDelivery.order).selectinload(Order.customer)
    )
    ...
    if delivery.status in [DeliveryStatus.DELIVERED, DeliveryStatus.FAILED]:
        await notification_service.send_notification(
            recipient={
                ...
                "email": delivery.order.customer.email if hasattr(delivery.order.customer, 'email') else None,
                "phone": delivery.order.customer.phone,
                "name": delivery.order.customer.name
            },
            event_type=NotificationType.DELIVERY_COMPLETED if delivery.status == DeliveryStatus.DELIVERED else NotificationType.DELIVERY_FAILED,
            context={
                "order_number": delivery.order.order_number,
                "customer_name": delivery.order.customer.name,
                "status": "已送達" if delivery.status == DeliveryStatus.DELIVERED else "配送失敗",
...
@router.post("/deliveries/confirm/{delivery_id}", response_model=DeliveryConfirmResponse)
async def confirm_delivery(
    ...
    if signature:
        ...
        signature_data = base64.b64decode(signature.split(',')[1] if ',' in signature else signature)
        signature_path = Path(f"uploads/signatures/{delivery_id}_{datetime.utcnow().timestamp()}.png")
    ...
    if photo:
        photo_path = Path(f"uploads/delivery_photos/{delivery_id}_{datetime.utcnow().timestamp()}_{photo.filename}")
...
async def sync_offline_data(
    ...
    for location in sync_data.locations:
        try:
            ...
            synced_items.append({"type": "location", "id": location.get("id"), "status": "synced"})
        except Exception as e:
            failed_items.append({"type": "location", "id": location.get("id"), "error": str(e)})
        ...
        try:
            ...
            if delivery:
                ...
                if "delivered_at" in delivery_update:
                    delivery.delivered_at = datetime.fromisoformat(delivery_update["delivered_at"])
                ...
                synced_items.append({"type": "delivery", "id": str(delivery_id), "status": "synced"})
            else:
                failed_items.append({"type": "delivery", "id": str(delivery_id), "error": "Delivery not found"})
                
        except Exception as e:
            failed_items.append({"type": "delivery", "id": str(delivery_update.get("delivery_id")), "error": str(e)})
