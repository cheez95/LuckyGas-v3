...
from app.schemas.sms import (
    SMSLogResponse, SMSStatsResponse, SMSTemplateResponse,
    ProviderConfigResponse, SMSSendRequest, SMSResendRequest,
    SMSTemplateCreate, SMSTemplateUpdate, ProviderConfigUpdate
)
...
async def get_sms_stats(
    ...
    result = await db.execute(
        select(
            ...
            func.sum(
                func.cast(
                    SMSLog.status == NotificationStatus.DELIVERED,
                    Integer
                )
            ).label("delivered"),
            func.sum(
                func.cast(
                    SMSLog.status == NotificationStatus.FAILED,
                    Integer
                )
            ).label("failed"),
            func.sum(
                func.cast(
                    SMSLog.status == NotificationStatus.PENDING,
                    Integer
                )
            ).label("pending"),
...
async def resend_sms(
    ...
    if original.status != NotificationStatus.FAILED:
        raise HTTPException(
            status_code=400,
            detail="Can only resend failed messages"
        )
    ...
    result = await enhanced_sms_service.send_sms(
        ...
        metadata={
            "resend_of": str(original.id),
            **original.notification_metadata
        },
        db=db
    )
