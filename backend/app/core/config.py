...
class BusinessConfig(BaseModel):
    ...
    delivery_start_hour: int = Field(
        8, ge=0, le=23, description="Earliest delivery hour"
    )
    ...
    driver_cost_per_hour: float = Field(
        500.0, gt=0, description="Driver cost per hour (TWD)"
    )
    fuel_cost_per_km: float = Field(
        10.0, gt=0, description="Fuel cost per kilometer (TWD)"
    )
    ...
    weekend_surcharge: float = Field(
        1.2, ge=1, description="Weekend delivery surcharge multiplier"
    )
...
class SecurityConfig(BaseModel):
    ...
    password_require_special: bool = Field(
        True, description="Require special character"
    )

    # Session settings
    session_timeout_minutes: int = Field(
        1440, ge=1, description="Session timeout (24 hours)"
    )
    max_login_attempts: int = Field(5, ge=1, description="Maximum login attempts")
    lockout_duration_minutes: int = Field(
        30, ge=1, description="Account lockout duration"
    )
...
class DatabaseConfig(BaseModel):
    ...
    # Pool settings
    pool_size: int = Field(
        20, ge=1, le=100, description="Number of connections to maintain"
    )
    max_overflow: int = Field(
        10, ge=0, le=50, description="Maximum overflow connections"
    )
    pool_timeout: int = Field(30, ge=1, description="Timeout waiting for connection")
    pool_recycle: int = Field(
        3600, ge=60, description="Recycle connections after seconds"
    )
    pool_pre_ping: bool = Field(True, description="Test connections before using")

    # Performance settings
    statement_timeout: int = Field(
        60000, ge=1000, description="Statement timeout in milliseconds"
    )
    ...
    keepalives_idle: int = Field(
        30, ge=1, description="Seconds before sending keepalive"
    )
    keepalives_interval: int = Field(5, ge=1, description="Interval between keepalives")
    keepalives_count: int = Field(
        5, ge=1, description="Number of keepalives before closing"
    )
...
class Settings(BaseSettings):
    ...
    def validate_environment(cls, v: str) -> Environment:
        try:
            ...
            raise ValueError(
                f"Invalid environment: {v}. Must be one of: {', '.join([e.value for e in Environment])}"
            )
    ...
    FIRST_SUPERUSER: str = Field(
        "admin@luckygas.tw", description="Initial superuser email"
    )
    FIRST_SUPERUSER_PASSWORD: str = Field(
        ..., min_length=8, description="Initial superuser password"
    )
    ...
    def validate_taiwan_phone(phone: str) -> bool:
        ...
        return bool(
            ...
            or re.match(landline_pattern, phone_clean)
        )
