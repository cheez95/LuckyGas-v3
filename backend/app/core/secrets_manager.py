...
class SecretsManager:
    ...
    def __init__(self, project_id: Optional[str] = None):
        ...
        self._use_secret_manager = bool(self.project_id and os.getenv("GOOGLE_APPLICATION_CREDENTIALS"))
        
        if self._use_secret_manager:
            try:
                self._client = secretmanager.SecretManagerServiceClient()
                logger.info(f"Initialized Secret Manager for project: {self.project_id}")
            except Exception as e:
                logger.warning(f"Failed to initialize Secret Manager: {e}. Falling back to environment variables.")
                self._use_secret_manager = False
        else:
            logger.info("Using environment variables for secrets (Secret Manager not configured)")
    ...
    def get_secret_json(self, secret_id: str, version: str = "latest") -> Optional[Dict[str, Any]]:
        """
...
def get_secret_json(secret_id: str, version: str = "latest") -> Optional[Dict[str, Any]]:
