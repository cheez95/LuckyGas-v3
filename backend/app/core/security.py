...
from app.core.security_config import (
    get_password_policy, get_session_config, get_lockout_policy,
    get_api_key_config, get_2fa_config
)
...
class PasswordValidator:
    ...
    async def update_password_history(user_id: int, password_hash: str):
        ...
        await cache.set(history_key, str(history_list), expire=86400 * 365)  # Keep for 1 year
...
class AccountLockout:
    ...
    async def record_failed_attempt(identifier: str, is_ip: bool = False):
        ...
        # Store attempt count
        await cache.set(f"{key}:attempts", str(attempts), expire=3600)  # Reset after 1 hour
        ...
        if attempts >= max_attempts:
            ...
            await cache.set(f"{key}:count", str(lockout_count), expire=86400 * 30)  # Remember for 30 days
...
class TwoFactorAuth:
    ...
    def generate_backup_codes(count: int = None) -> List[str]:
        ...
        for _ in range(count):
            code = ''.join(secrets.choice('0123456789') for _ in range(config.backup_code_length))
    ...
    async def send_sms_code(phone: str) -> str:
        ...
        # Generate code
        code = ''.join(secrets.choice('0123456789') for _ in range(config.sms_code_length))
...
class SessionManager:
    ...
    async def create_session(user_id: int, remember_me: bool = False) -> Dict[str, Any]:
        ...
        refresh_token_expires = timedelta(
            days=config.remember_me_days if remember_me else config.refresh_token_expire_days
        )
        ...
        await cache.set(f"session:{session_id}", str(session_data), expire=config.session_timeout_minutes * 60)
    ...
    async def validate_session(session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if datetime.utcnow() - last_activity > timedelta(minutes=config.idle_timeout_minutes):
            await SessionManager.revoke_session(session_id)
            return None
        
        # Update last activity
        session["last_activity"] = datetime.utcnow().isoformat()
        await cache.set(f"session:{session_id}", str(session), expire=config.session_timeout_minutes * 60)
