...
class SecurityConfig:
    ...
    def _get_password_policy(self) -> PasswordPolicy:
        ...
        if self.security_level == SecurityLevel.HIGH:
            ...
            return PasswordPolicy(min_length=10, history_count=5, max_age_days=90)
    ...
    def _get_lockout_policy(self) -> AccountLockoutPolicy:
        ...
        if self.security_level == SecurityLevel.HIGH:
            ...
            return AccountLockoutPolicy(max_attempts=5, lockout_duration_minutes=30)
        else:  # LOW
            return AccountLockoutPolicy(
                enabled=False, max_attempts=10, lockout_duration_minutes=5
            )
    ...
    def _get_2fa_config(self) -> TwoFactorConfig:
        ...
        if self.security_level == SecurityLevel.HIGH:
            return TwoFactorConfig(
                enabled=True, sms_enabled=True, trusted_device_days=7
            )
        elif self.security_level == SecurityLevel.MEDIUM:
            return TwoFactorConfig(
                enabled=True, sms_enabled=False, trusted_device_days=30
            )
        else:  # LOW
