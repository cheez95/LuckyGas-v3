...
def cache_response(expire_seconds: int = 300, key_prefix: Optional[str] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            if isinstance(result, dict) or (hasattr(result, "status_code") and result.status_code < 400):
                # Convert response to cacheable format
                if isinstance(result, dict):
                    cache_data = json.dumps(result)
                else:
                    # For Response objects, we need to extract the body
                    cache_data = result.body.decode() if hasattr(result, "body") else json.dumps(result)
                ...
                if hasattr(result, "headers"):
                    ...
                    result.headers["Cache-Control"] = f"private, max-age={expire_seconds}"
