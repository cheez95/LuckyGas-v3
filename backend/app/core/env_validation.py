...
class EnvironmentValidator:
    ...
    def validate(cls) -> Tuple[bool, List[str]]:
        ...
        for var, description in cls.OPTIONAL_VARS.items():
            if not os.getenv(var):
                ...
                if environment == "production" and var in ["GOOGLE_API_KEY", "GOOGLE_APPLICATION_CREDENTIALS"]:
                    errors.append(f"Missing critical variable for production: {var} - {description}")
                else:
    ...
    def _validate_specific_formats(cls, errors: List[str]):
        ...
        if log_level not in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
            errors.append(f"Invalid LOG_LEVEL: {log_level}. Must be one of: DEBUG, INFO, WARNING, ERROR, CRITICAL")
        
        # Validate ENVIRONMENT
        environment = os.getenv("ENVIRONMENT", "development").lower()
        if environment not in ["development", "staging", "production", "test"]:
            errors.append(f"Invalid ENVIRONMENT: {environment}. Must be one of: development, staging, production, test")
        ...
        if dev_mode not in ["auto", "production", "development", "offline"]:
            errors.append(f"Invalid DEVELOPMENT_MODE: {dev_mode}. Must be one of: auto, production, development, offline")
        
        # Validate numeric thresholds
        for var in ["DAILY_COST_WARNING", "DAILY_COST_CRITICAL", "ROUTES_RATE_LIMIT_PER_SECOND"]:
            value = os.getenv(var)
            if value:
                try:
                    float(value)
