...
class CustomerRepository(CachedRepository[Customer]):
    ...
    async def get_active_customers(
        ...
        count_query = select(func.count()).select_from(Customer).where(Customer.is_terminated == False)
    ...
    async def get_customer_with_recent_orders(
        ...
        query = (
            ...
            .where(Customer.id == customer_id)
            .options(
                selectinload(Customer.orders).options(
    ...
    async def get_high_value_customers(
        self,
        min_monthly_orders: int = 4,
        min_avg_order_value: float = 5000.0
    ) -> List[Customer]:
        """
    ...
    async def bulk_update_areas(self, area_updates: List[Dict[str, Any]]) -> int:
        ...
        stmt = (
            ...
            .where(Customer.id == func.any_(
                [update['id'] for update in area_updates]
            ))
        )
